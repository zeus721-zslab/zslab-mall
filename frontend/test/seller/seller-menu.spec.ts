...
// 메뉴 구조(대시보드/주문/상품/통계/정산/설정)와 활성 판정. 90-B-3 대시보드·주문·배송·정산 + 90-C-3 상품·재고 + 90-D-1 클레임 + 90-D-2 비밀번호 변경 + 90-D-3 정산계좌(설정 그룹) + 90-E-1 통계 매출이 경로를 가지며 통계 주문·클레임·상품은 비활성(to 없음·라우트 미생성).
describe('SELLER_MENU', () => {
  it('순서 고정 · 대시보드·주문·배송·클레임·상품·재고·통계 매출·정산·비밀번호 변경·정산계좌만 경로 보유 · 통계 나머지 2개는 비활성', () => {
    ...
    expect(paths).toEqual(['/seller', '/seller/orders', '/seller/deliveries', '/seller/claims', '/seller/products', '/seller/products/inventory', '/seller/stats/sales', '/seller/settlements', '/seller/settings/password', '/seller/settings/bank-account'])
    const statsChildren = SELLER_MENU.find((group) => group.label === '통계')?.children ?? []
    expect(statsChildren.map((child) => [child.label, child.to ?? null])).toEqual([['매출', '/seller/stats/sales'], ['주문·클레임', null], ['상품', null]])
  })
