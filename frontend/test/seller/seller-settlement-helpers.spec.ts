...
// Track 90-B-3(Track 85 셀러 API): 읽기 전용 표시 규칙 + 확정 전(PENDING 404) 안내 문구.
describe('seller-settlement-view', () => {
  it('기간·날짜·수수료율·상태 라벨', () => {
    ...
    expect(SELLER_SETTLEMENT_STATUS_LABEL.PENDING).toBe('확정 전')
  })
  ...
  it('pendingSettlementNotice: 0건 null · N건이면 대시보드 "정산 예정"과 같은 건수임을 설명', () => {
    ...
    expect(pendingSettlementNotice(1)).toContain('확정 전 정산 1건')
