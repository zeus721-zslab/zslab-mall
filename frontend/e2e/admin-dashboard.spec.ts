 ...
 * 요약 카드 6·처리 대기 9·차트 2(apexcharts SVG)·리스트 4가 렌더되는지 확인한다. 데이터 유무와 무관하게 성립하는 단언만 둔다.
 * 클레임 처리 대기 타일(Track 96-4)은 클릭 → 클레임 목록 ?action=FOLLOWUP 복원·필터 select 표시까지 확인한다.
 */
test.describe('관리자 대시보드 (FE-33)', () => {
  test('① 로그인 → /admin: 요약 카드 6·증감 배지·처리 대기 9·차트 2·리스트 4 렌더', async ({ page }) => {
    ...
    await expect(page.getByTestId('dashboard-pending-count')).toHaveCount(9)
    ...
    await expect(page.getByTestId('dashboard-pending-reconciliationOpen')).toHaveAttribute('href', '/admin/orders/reconciliation?status=OPEN')
    // Track 102 FE-64: 칸마다 "무엇을 센 건지 · 어디서 처리하는지" 한 줄. 근사 집계 3칸은 건수 차이를 알린다.
    await expect(page.getByTestId('dashboard-pending-hint')).toHaveCount(9)
