 ...
 * 요약 카드 6·처리 대기 8·차트 2(apexcharts SVG)·리스트 4가 렌더되는지 확인한다. 데이터 유무와 무관하게 성립하는 단언만 둔다.
 * 클레임 처리 대기 타일(Track 96-4)은 클릭 → 클레임 목록 ?action=FOLLOWUP 복원·필터 select 표시까지 확인한다.
 */
test.describe('관리자 대시보드 (FE-33)', () => {
  test('① 로그인 → /admin: 요약 카드 6·증감 배지·처리 대기 8·차트 2·리스트 4 렌더', async ({ page }) => {
    ...
    // 처리 대기 8칸: 링크 8(정산·클레임·배송·재고 임박=상품 목록 stockFilter=LOW·Track 89-A / 상품·셀러 승인 대기=status=PENDING·Track 96-2 /
    // 클레임 처리 대기=action=FOLLOWUP·Track 96-4 / 장기 배송중=배송 목록 status=SHIPPING·Track 99 D-210)
    await expect(page.getByTestId('dashboard-pending-count')).toHaveCount(8)
