 ...
 * ① 진입점 안내 → 행·상태 chip → scope 필터(URL·API) → 행 메뉴(SHIPPING만) → 배송완료 성공 ② 송장 정정 409 → 필드 오류 유지 → 재시도 성공.
 */
...
async function mockSellerDeliveries(page: Page, options: { conflictFirstPatch?: boolean } = {}): Promise<Captured> {
  ...
  await page.route((url) => /\/api\/v1\/seller\/deliveries\/dlv_[^/]+\/tracking$/.test(url.pathname), (route) => {
    ...
    if (options.conflictFirstPatch && captured.patches.length === 1) {
      return route.fulfill({ status: 409, json: { type: 'about:blank', title: 'Conflict', status: 409, code: 'DELIVERY_TRACKING_NO_CONFLICT', detail: '중복' } })
    }
...
test.describe('셀러 배송 화면(90-B-3)', () => {
  ...
  test('② 송장 정정: 현재 값 프리필·사유 필수 → PATCH 409 → 송장번호 필드 오류(다이얼로그 유지) → 번호 변경 후 재시도 200 → 토스트', async ({ page }) => {
    const captured = await mockSellerDeliveries(page, { conflictFirstPatch: true })
    await loginAs(page, 'SELLER')
    await page.setViewportSize({ width: 1440, height: 900 })
    await page.goto('/seller/deliveries')
    ...
    await expect(dialog).toContainText('다른 배송이 이미 사용 중인 송장번호입니다.')
