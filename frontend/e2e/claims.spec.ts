...
const ORDER_DETAIL = {
  ...
  sellers: [{
    ...
    items: [
      ...
      { orderItemId: DELIVERED_ITEM, productName: 'E2E 배송완료 양말', quantity: 1, unitPrice: 10000, totalPrice: 10000, status: { code: 'DELIVERED', label: 'DELIVERED' } },
    ],
  }],
  totalPrice: 29900,
...
test.describe('사용자 반품 요청·회수·검수(FE-29)', () => {
  test('① 주문 상세: 배송중 품목 반품 버튼 없음·배송완료만 → 반품 요청 사유 3값 → 불량 선택 시 첨부 섹션·2장 업로드(순서) → 제출 body attachmentIds / 단순변심으로 바꾸면 첨부 해제 / 스크린샷', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: '반품 요청' })).toHaveCount(1)
