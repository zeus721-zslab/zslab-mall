...
test.describe('사용자 반품 요청·회수·검수(FE-29)', () => {
  test('① 주문 상세: 배송중 품목 반품 버튼 없음·배송완료만 → 반품 요청 사유 3값 → 불량 선택 시 첨부 섹션·2장 업로드(순서) → 제출 body attachmentIds / 단순변심으로 바꾸면 첨부 해제 / 스크린샷', async ({ page }) => {
    ...
    await expect(page.getByTestId('item-product-name').filter({ hasText: 'E2E 배송완료 양말' })).toBeVisible()
    await expect(page.getByTestId('item-claim-return')).toHaveCount(2) // 배송완료 + 교환 완료(반품만) 품목
    await expect(page.getByTestId('item-claim-exchange')).toHaveCount(1)
    await expect(page.getByTestId('item-claim-cancel')).toHaveCount(0)
    ...
    await expect(page.getByTestId('item-confirm-warning')).toContainText('확정하면 이 품목의 반품·교환을 더 이상 요청할 수 없습니다.')
    ...
    await page.getByTestId('item-claim-return').first().click()
    await page.waitForURL(/\/claims\/new\?/)
    expect(page.url()).toContain(`orderItem=${DELIVERED_ITEM}`)
    const options = page.getByTestId('claim-reason-option')
    ...
    await page.getByTestId('claim-reason-code').selectOption('PRODUCT_DEFECT')
    ...
    await page.getByTestId('claim-reason-code').selectOption('BUYER_CHANGED_MIND')
    await expect(page.getByTestId('claim-attachment-input')).toHaveCount(0)
    await page.getByTestId('claim-reason-code').selectOption('WRONG_PRODUCT')
  ...
  test('② 클레임 상세(승인·회수 송장 대기): 6단 타임라인·첨부 사진 → 택배사·송장 등록 → POST body → 재조회로 폼 사라짐·회수 송장 표기', async ({ page }) => {
    ...
    const steps = page.getByTestId('claim-timeline-step')
    ...
    await expect(page.getByTestId('claim-attachments').getByTestId('claim-attachment-photo')).toHaveCount(1)
    ...
    await form.getByTestId('claim-return-shipment-carrier').selectOption('CJ')
    await form.getByTestId('claim-return-shipment-tracking-no').fill('RTN-0001')
    await form.getByTestId('claim-return-shipment-submit').click()
  ...
  test('④ 교환(FE-30): 교환 완료 품목은 교환 버튼 없음·반품만 → 교환 요청 사유 3값·옵션 후보(같은 가격·품절 제외·현재 제외) 1건·미선택 제출 불가 → 불량 첨부 → body exchangeVariantId', async ({ page }) => {
    ...
    await loginAs(page, 'BUYER')
    await gotoClientSide(page, `/orders/${ORDER_ID}`)
    await expect(page.getByTestId('item-product-name').filter({ hasText: 'E2E 교환완료 모자' })).toBeVisible()
    await expect(page.getByTestId('item-claim-exchange')).toHaveCount(1) // 교환 완료 품목은 숨김
    await expect(page.getByTestId('item-claim-return')).toHaveCount(2)

    await page.getByTestId('item-claim-exchange').click()
    await page.waitForURL(/\/claims\/new\?/)
    ...
    await expect(page.getByTestId('claim-type-guidance')).toContainText('같은 가격의 다른 옵션으로만 교환됩니다')
    const options = page.getByTestId('claim-reason-option')
    await expect(options).toHaveCount(3)
  ...
  test('⑤ 교환 클레임 상세(FE-30·승인·회수 송장 대기): 7단 타임라인·교환 옵션 행·교환 회수 안내 / 스크린샷', async ({ page }) => {
    ...
    await expect(steps.nth(0)).toContainText('요청')
    await expect(steps.nth(1)).toContainText('승인')
  ...
  test('⑥ 교환 클레임 상세(FE-30·완료): 7단 완료·교환품 배송 송장·회수 폼 없음 / 스크린샷', async ({ page }) => {
    ...
    await loginAs(page, 'BUYER')
    await gotoClientSide(page, `/claims/${CLAIM_ID}`)
    await expect(page.getByTestId('claim-timeline-step')).toHaveCount(7)
    await expect(page.getByTestId('claim-timeline-step').nth(6)).toContainText('완료')
    await expect(page.getByTestId('claim-reshipment')).toContainText('한진택배 EXC-0001')
    await expect(page.getByTestId('claim-reshipment-label')).toHaveText('교환품 배송 송장')
    await expect(page.getByTestId('claim-return-shipment-form')).toHaveCount(0)
