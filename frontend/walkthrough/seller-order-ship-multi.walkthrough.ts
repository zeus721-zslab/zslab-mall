...
/**
 * 셀러 시나리오 3(Track 99·다건): 배송 대기 품목 3건을 연속으로 발송 처리한다.
...
test('셀러 · 배송 대기 3건 연속 발송', async ({ page }) => {
  const walkthrough = new Walkthrough(page, 'seller', 'order-ship-multi', '배송 대기 3건 연속 발송')
  ...
  for (let index = 1; index <= SHIP_COUNT; index += 1) {
    ...
    await walkthrough.shot(index + '건째-발송-다이얼로그')

    // Track 99 FE-61: 직전 발송 택배사가 기본 선택돼 2건째부터는 택배사를 다시 고르지 않는다(1건째만 고른다).
    if (index === 1) {
