...
/**
 * 셀러 시나리오 1(Track 98): 대시보드 배송 대기에서 신규 주문 품목 1건을 출고(송장 등록)한다.
 * 완료 조건 = 해당 행의 품목 상태 칩이 '배송중'.
 */
test('셀러 · 신규 주문 확인 → 출고(송장 등록)', async ({ page }) => {
  const walkthrough = new Walkthrough(page, 'seller', 'order-ship', '신규 주문 → 출고(송장 등록)')
  ...
  await walkthrough.shot('출고-다이얼로그')
  ...
  await walkthrough.shot('출고-정보-입력')
