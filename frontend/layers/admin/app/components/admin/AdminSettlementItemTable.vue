 ...
 * 주문번호 클릭은 주문 상세로만 이동한다(orderPublicId·D-134). 이월 차감(CARRYOVER) 행은 주문 품목이 없어 주문·상품·수량을 "—"로 둔다.
...
<template>
  ...
  >
    ...
    <template #[`item.orderPublicId`]="{ item }">
      <a v-if="item.orderPublicId" href="#" class="font-weight-medium text-primary text-decoration-none adm-product-id" data-testid="item-order-no" @click.prevent="emit('open', item)">{{ item.orderPublicId }}</a>
