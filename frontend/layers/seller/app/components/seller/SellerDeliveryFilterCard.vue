...
import type { SellerDeliveryListQuery } from '#layers/seller/app/types/seller-delivery'
import { SELLER_DELIVERY_CARRIER_OPTIONS, SELLER_DELIVERY_STATUS_OPTIONS } from '#layers/seller/app/lib/constants/seller-order'
import {
  SELLER_DELIVERY_KEYWORD_MAX,
  SELLER_DELIVERY_SCOPE_OPTIONS,
...
// 배송 필터 카드(Track 90-B-3·관리자 AdminDeliveryFilterCard 복제). 검색어는 로컬 입력값을 두고 검색 버튼·Enter로만 확정한다.
// 기간(발송일·네이티브 date 2개)·드롭다운(범위·상태·택배사·정렬)은 변경 즉시 확정. 모든 확정은 emit('apply')로 부모(URL 단일 소스)에 넘긴다.
const props = defineProps<{
  query: SellerDeliveryListQuery
}>()

const emit = defineEmits<{
  apply: [patch: Partial<SellerDeliveryListQuery>]
  reset: []
}>()
...
<template>
  <v-card class="mb-4" data-testid="seller-delivery-filters">
    <v-card-text class="pa-4">
      <v-row dense align="center">
        <v-col cols="12" md="6">
          <v-text-field
            ...
            :maxlength="SELLER_DELIVERY_KEYWORD_MAX"
            hide-details
            clearable
        ...
        <v-col cols="12" sm="4">
          <v-select
            ...
            :items="SELLER_DELIVERY_SCOPE_OPTIONS"
            label="조회 범위"
            hide-details
        ...
        <v-col cols="6" sm="4">
          <v-select
            ...
            :items="SELLER_DELIVERY_STATUS_OPTIONS"
            label="배송상태"
            hide-details
            clearable
      ...
      <div class="d-flex align-center justify-space-between flex-wrap ga-2 mt-3">
        ...
        <!-- 정렬 select는 버튼 줄 인라인이라 compact(의도된 예외·관리자 동일) -->
        <v-select
          :model-value="query.sort"
          :items="SELLER_DELIVERY_SORT_OPTIONS"
          label="정렬"
          hide-details
          density="compact"
