...
import type { SellerStatsSummaryCardView } from '#layers/seller/app/lib/seller-stats-view'
import { STATS_COMPARE_LABELS, type StatsCompare } from '~/lib/constants/stats'

/**
 * 셀러 통계 요약 카드 공용(Track 90-E-2·관리자 AdminStatsSummaryCards 복제). 카드 뷰(값·비교값·증감 배지 클래스)는 lib 순수 함수가 만들고 여기는
 * 배선만 한다. 비교 없으면 배지 "—"·회색·캡션은 NBSP로 줄을 차지시켜 높이를 맞춘다. 아이콘·색은 카드 key별 맵으로 받는다.
 */
const props = defineProps<{
  cards: SellerStatsSummaryCardView[]
