...
/**
 * 셀러 차트 래퍼(Track 90-B-3·관리자 AdminChart 복제). apexcharts는 window 의존이라 클라이언트에서만 동적 import한다(셀러 레이어 한정 로딩·
 * 사용자 entry 번들 무오염). 페이지는 이 컴포넌트만 쓰고 vue3-apexcharts를 직접 import하지 않는다. 옵션은 호출부가 lib/seller-dashboard-view.ts
 * 순수 함수로 만들어 넘긴다.
 *
 * 트랩(관리자 FE-33 실측): vue3-apexcharts 1.11은 options 변경 시 JSON 깊은 복사로 updateOptions를 호출해 formatter 함수가 사라진다(최초 init은 함수 보존).
