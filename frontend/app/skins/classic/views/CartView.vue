...
<template>
  <div class="py-8 md:py-12">
    <div class="mx-auto max-w-[1240px] px-4 md:px-6">
      ...
      <!-- 빈 장바구니 -->
      <CommonEmptyState v-else-if="vm.items.length === 0" message="장바구니가 비어 있습니다" />
      ...
      <div v-else class="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_320px]">
        <div class="space-y-4">
          ...
          <div class="flex items-center justify-between border-b border-line pb-3">
            <label class="flex items-center gap-2 text-sm font-medium text-ink">
              <input
                ...
                :disabled="vm.busy"
                @change="vm.toggleSelectedAll(($event.target as HTMLInputElement).checked)"
              />
              전체 선택
            </label>
          </div>
          ...
          <!-- 조작 오류 -->
          <p v-if="vm.opErrorMessage" role="alert" class="text-sm text-soldout">{{ vm.opErrorMessage }}</p>
          ...
          >
            ...
            <div class="flex items-start pt-1">
              <input
                ...
                @change="vm.toggleSelected(item, ($event.target as HTMLInputElement).checked)"
              />
            </div>
            ...
            <div class="flex min-w-0 flex-1 flex-col gap-2">
              ...
              <div class="mt-auto flex items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-3">
                  <span class="text-sm font-bold text-price">{{ vm.formatPrice(item.displayPrice * item.quantity) }}</span>
                  <button
                    type="button"
        ...
        <aside class="h-fit rounded-card border border-line p-5 lg:sticky lg:top-24">
          <div class="flex items-center justify-between">
            ...
            <span class="text-xl font-bold text-price">{{ vm.formatPrice(vm.selectedTotal) }}</span>
          </div>
          <!-- 구매 불가 선택 품목 포함 안내(FE-18): 결제 차단·삭제 유도 -->
          <p v-if="vm.hasUnpurchasableSelected" role="alert" class="mt-3 text-sm text-soldout">
            구매할 수 없는 상품이 포함되어 있습니다. 삭제 후 결제해 주세요.
          </p>
          <!-- 결제: 선택 품목이 있고 구매 불가 선택 품목이 없으면 체크아웃 페이지로 이동 -->
          <Button size="lg" class="mt-4 w-full" :disabled="!vm.checkoutEnabled" @click="vm.handleCheckout">
            결제하기
          </Button>
        </aside>
      </div>
