...
const CARD = 'max-w-[720px] rounded-card bg-white p-5 shadow-e1 md:p-6'
    ...
    <section :class="CARD" aria-label="회원 정보 수정">
      ...
      <div v-if="vm.pending" class="space-y-5" aria-hidden="true">
        <div v-for="index in 3" :key="index" class="space-y-2">
          ...
          <div class="h-12 rounded-control bg-surface-muted"></div>
        </div>
      </div>
      ...
      <form v-else class="space-y-5" @submit.prevent="vm.handleSubmit">
        <div>
          ...
          <input
            ...
            type="email"
            readonly
            class="h-12 w-full cursor-not-allowed rounded-control border border-line bg-surface-page px-4 text-body text-sub"
          />
        </div>
        ...
        <button type="submit" class="btn btn-primary btn-lg w-full sm:w-48" :disabled="vm.submitting">
