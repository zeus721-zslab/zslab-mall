...
// renew 주문 완료(FE-71 · FE-78). 결과 띠 면(민트): 흰 원형 체크 · 안내 · 주문번호(식별자라 mono). 주문 id가 없으면(직접 진입 등) classic처럼 주문 목록으로 유도한다.
...
<template>
  <div class="px-5 pb-8 pt-10 md:pt-16">
    <div class="mx-auto max-w-[480px] rounded-(--panel-radius) bg-(--pastel-mint-bg) px-6 py-10 text-center md:px-10">
      ...
      <!-- 주문번호 = 사람이 읽는 orderNo(모노 칩) · 조회 전·실패·옛 응답이면 생략(내부 id 노출 금지 · Track 105-4g-3) -->
      <div v-if="vm.orderNo" class="mt-8 rounded-card bg-white px-5 py-4">
        <p class="text-caption font-normal text-sub">주문번호</p>
        <p class="mt-1"><span :class="ORDER_NO_CHIP_CLASS" data-testid="checkout-complete-order-id">{{ vm.orderNo }}</span></p>
      </div>
