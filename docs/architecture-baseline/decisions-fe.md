...
- 보충(2026-09-26 · P2 후 재측정 · 성능 트랙 종료)
  - 운영 모바일 재측정(Lighthouse 3회 중앙값 · 5화면): 성능 87 유지(회차 범위 82.3~89.1) · 6축 평균 95.8 → 트랙 완료 기준(성능 82 · 평균 95) 충족으로 성능 트랙을 종료한다.
  - P2 효과: 이미지 요청 시작은 당겨졌다(Load Delay /products 3,205 → 1,925ms · 상세 3,214 → 1,652ms, −1.3~1.6s). 대신 Load Time이 늘어(/products 308 → 1,747ms · 상세 315 → 1,564ms) LCP 합계는 그대로다(/products 4.59 → 5.04s · 상세 4.64 → 4.60s). 4G 시뮬레이션에서 폰트·JS와 대역폭을 나눈 결과로 추정한다. 실제 요청 우선순위는 Low → High로 바뀌었으므로 P2는 되돌리지 않고 유지한다.
  - 이월: P3 JS 분할 · 렌더 차단 CSS · 폰트 전송량(위 §8 그대로).
  - README 대표 화면(docs/readme)과 성능 수치(71 → 87 · 95.8)에 반영했다.
