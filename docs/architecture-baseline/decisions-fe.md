...
# FE-12c 설계 확정본 — 미결제 주문 종료 모델 재설계 (D-153 갱신·레벨3 통합 이행)
...
## 상태 모델
- Order.status 신규 = PAYMENT_EXPIRED (결제 미완 종료·비노출). Order가 직접 세팅(PENDING_PAYMENT 선례·Resolver 미경유·OrderItem 무변경).
- Payment.status 신규 = EXPIRED (PENDING→EXPIRED 신규 전이·FAILED와 분리·"실패 아닌 유효기간 종료된 미완료 결제").
- CANCELLED = 결제 후 사용자 취소(Claim 경로)로 존치. cancel_reason 컬럼은 운영자 취소 실존 시 도입(현재 이연).

## 전체 흐름
주문 생성 → PENDING_PAYMENT (유효·노출·재고 예약)
  ├ 결제 성공 → PAID  [승인 전 Order.status==PENDING_PAYMENT 확인 = 늦은 웹훅 차단 불변식]
  ├ 결제 후 사용자 취소 → Claim(취소) → Order CANCELLED (노출·유지)
  └ 결제 미완(결제창 이탈·PG오류·30분 방치·시작 실패)
       → Order = PAYMENT_EXPIRED (비노출)
       → Payment = EXPIRED
       → OrderTerminated 발행 → 재고 해제
       → [FE-12c-2] 삭제 가능 확인 후 hard delete (재고 해제 선행)

## FE-12c 작업 범위 (삭제 배치 제외)
1. 마이그레이션 2건: order.status ENUM +PAYMENT_EXPIRED / payment.status ENUM +EXPIRED (신규 Vn·V1 수정 금지).
2. Java enum: OrderStatus +PAYMENT_EXPIRED / PaymentStatus +EXPIRED + canTransitionTo에 PENDING→EXPIRED 추가.
3. auto-cancel 재배선: OrderAutoCancelService가 CANCELLED 대신 PAYMENT_EXPIRED로 종료. Order 직접 세팅 방식(Resolver 규칙 미신설).
4. 이벤트 개명: OrderCancelled → OrderTerminated. InventoryOrderCancelledHandler 구독 대상 변경(release 로직 무변경).
5. InventoryPaymentFailedHandler 제거 → 재고 해제를 OrderTerminated 단일 구독으로 수렴.
6. handleCancel(결제창 취소)·expireOne(만료) 재배선: Payment를 EXPIRED로 종료 + Order를 PAYMENT_EXPIRED로 종료 + OrderTerminated 발행. PaymentFailed 발행 결합 제거.
7. 늦은 웹훅 차단 불변식: 결제 성공 승인 전 Order.status==PENDING_PAYMENT 확인(후보 = onPaymentCompleted 또는 markPaid 지점).
8. Payment 만료 스케줄러 정리: expireOne이 Order 종료 경유로 통일(역할 중복 해소).
9. FE: OrderStatusCode 유니온 +PAYMENT_EXPIRED / 목록 조회에서 PAYMENT_EXPIRED 제외(비노출) / 라벨. PaymentStatus는 FE 유니온 부재(느슨 문자열)라 표시 로직만 점검.

## FE-12c-2로 분리 (후속 트랙)
- PAYMENT_EXPIRED 주문 일 배치 hard delete. 자식(order_item·snapshot·payment) 순차 삭제 + idempotency_key dangling 정리. 삭제 가능 조건 확인 후 실행. 재고 해제 선행 필수.
- 분리 사유: 되돌릴 수 없는 hard delete는 상태 전이가 실측 안정된 뒤 얹음. 없어도 시스템 정상(재고 해제됨·가비지만 잔존).

## D-153 갱신 대상
- Phase 2·3·4를 본 트랙으로 통합. "시간초과→CANCELLED 수렴"을 "→PAYMENT_EXPIRED 분리"로 정정.
- OrderCancelled→OrderTerminated 개명 반영. Payment.EXPIRED 신설로 §8 "PENDING→CANCELLED 불법·Phase 3 재검토" 항목 해소.
