...
**박제 1줄**: "관측성 컨텍스트 전파 표준 = MDC 키 `traceId`·`correlationId`·`eventName` 3종·event record 무수정 (D-30 사실 통지 정합)·eventId 도입은 Outbox 트리거 시점 동반."
...
2. **correlationId 누락 없음** — 대표 이벤트 흐름 (HTTP → publish → AFTER_COMMIT → 로그) 통합 테스트 1건으로 correlationId MDC 포함 단언
...
5. **failed 메트릭 증가 단언** — 통합 테스트 1건으로 `zslab.event.failed` 카운터 증가 단언·이벤트명·테스트 클래스명은 변경 가능
6. **Actuator scrape 확인** — `/actuator/prometheus` endpoint scrape 응답 본문에 `zslab.event.*` 메트릭 노출 확인

**종료조건 추가 금지** (WARN-16 흡수): 위 6건 외 추가 종료조건 신설 회피·과측정 위험·운영 효용 한계.
...
2. **핵심 진입점**:
   ...
   - `notification/handler/Notification*Handler.java` 7건 — 로그 prefix 적용 대상·structured log 5 표준키 적용 대상
   - `build.gradle.kts` — `micrometer-registry-prometheus` 의존성 추가 위치
   - `application.yml`·`application-prod.yml` — Actuator endpoint 노출 설정
3. **핵심 SoT 메서드**:
   - `ApplicationEventPublisher.publishEvent` 11 호출처 — Micrometer `zslab.event.published` 카운터 적용 대상
   - 핸들러 catch 블록 18건 — `zslab.event.failed` 카운터 + 로그 prefix 동반 적용 대상
4. **영향 범위**:
   - production: `common/web/`·`notification/handler/`·`build.gradle.kts`·`application*.yml`
   - test: 통합 테스트 ≥2건 (PR-1 correlationId·PR-2 failed metric)
...
| 14 (관측성 도입 완료 기준 부재) | P2 | §종료조건 6건 박제 | §종료조건 |
...
- (조건부 Q14 실측 결과) `backend/src/main/java/com/zslab/mall/common/observability/MdcContextCopier.java` — MDC 미상속 발견 시 신설·자연 상속 확인 시 미신설
...
- `backend/src/main/java/com/zslab/mall/notification/handler/NotificationOrderPlacedHandler.java` — `[Notification]` prefix + 5 표준키 적용
...
- `backend/src/main/java/com/zslab/mall/common/observability/EventMetricsRecorder.java` (또는 동등 구현) — `zslab.event.published`·`zslab.event.failed` 카운터 발화 위치
...
이벤트 record 11종 (D-30 정합·Q3 β′ 정합)·도메인 Service 11 publishEvent 호출처 (행위 무변경)·도메인 Aggregate·invariants·state-machine·aggregate-boundary·domain-events·live-traps·DDL/Flyway (스키마 무변경)·기존 NotificationService·기존 핸들러 18건 본문 (Notification 7건 prefix·structured log 5 표준키만 적용·로직 무변경)·동기 핸들러 4건 (OrderEventHandler·ClaimRequestedHandler·DeliveryStartedHandler·DeliveryCompletedHandler 전건 무변경)·기존 통합 테스트 20건 (회귀 0)·Aggregate boundary·SoT 박제 1줄 6건 (D-90·D-94 Q6·D-95 Q6·D-97 Q8·D-98 Q5·D-99 Q11).

#### 회귀 예상

PR-1: 433 baseline → 신규 합산 ≥435 (단위 ≥1 + 통합 ≥1·기존 회귀 0)
PR-2: PR-1 머지 후 baseline 기준 → 신규 합산 ≥436 (통합 ≥1·기존 회귀 0)
...
2. **PR-1** 브랜치 `feat/track-16-observability-standards` 생성 (main HEAD 422a92e 기준).
3. 가드 5 사전 통지: PR-1 신규 (조건부 1) + 신규 테스트 2 + 수정 production 8 (TraceIdFilter 1·Notification 7)·PR-2 수정 production 3 (build.gradle.kts·application.yml·application-prod.yml) + 신규 (EventMetricsRecorder 1) + 신규 테스트 1 — Claude Code 구현 프롬프트 시점 일괄 통지.
4. TDD 우선 (단위 → 통합)·BUILD SUCCESSFUL·PR-1 신규 ≥2 PASS·기존 회귀 0.
5. GitHub Web UI PR 생성·Base: main·Compare: feat/track-16-observability-standards·외부 검토 1·2·3차 완료·S급.
6. PR-1 머지 후 PR-2 진입·동일 절차.
7. PR-1·PR-2 양 머지로 Track 16 종결·D-99 §후속 Observability carry-over 영구 해소·박제 1줄 누적 6건 통합 재평가 완료·신규 박제 1줄 14건 추가·관측성 표준 박제 완료.
