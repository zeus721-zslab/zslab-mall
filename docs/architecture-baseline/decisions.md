...
## D-155. 미결제 종료 주문 삭제 배치·결제 시작 상태 가드 (FE-12c-2)

### §1 결정
PAYMENT_EXPIRED(미결제 종료) 주문을 유예 경과·재고 해제 완료 시 일 배치로 hard delete(가비지 정리). 삭제 안전성을 위해 PaymentService.initiate에 Order.status==PENDING_PAYMENT 가드를 추가해 종료 주문에 결제 자식이 새로 생기는 동시성 창을 차단한다. "삭제보다 안전 우선" — 조건 미충족 시 skip/이연.

### §1-A 채택/기각 근거
- **채택 — initiate PENDING_PAYMENT 가드(근본 안전장치)**: 삭제 대상(PAYMENT_EXPIRED) 주문에 retryPayment→initiate가 새 PENDING payment 자식을 만들면 fk_payment_order RESTRICT로 삭제가 반복 실패. 삭제 배치에 락을 얹는 대신 "PAYMENT_EXPIRED 주문은 새 결제를 생성할 수 없다"는 도메인 불변식으로 원천 차단(단순·견고). 재결제 정상 경로(INITIATE_FAILED·만료 PENDING 재시도 = PENDING_PAYMENT 유지)는 통과. 외부 검토 Q2 정합.
- **채택 — 신규 예외 OrderNotPendingPaymentException·422**: 코드베이스 관례상 "요청 well-formed·서버 상태로 처리 불가"는 전부 422(PaymentAlreadyCompleted·OrderNotPayable 등). "종료 주문 결제 시작 불가"는 상태 위반이므로 422. 400은 형식 오류 전용이라 부정확(기각). OrderNotPayableException 재사용은 그 예외의 도메인 의미(재결제 재검증) 오염이라 기각.
- **채택 — 삭제 가능 조건을 정책으로(reserved==0 결과 상태)**: (0)status 재확인 (1)PENDING payment 부재 (2)전 order_item reserved==0. "미처리 이벤트 없음"은 outbox/failed_event 테이블 부재로 직접 검사 불가 → reserved==0(재고 결과 상태)로 갈음. §8 한계 참조.
- **채택 — reserved>0 시 OrderTerminated 재발행+이연(원칙 3 준수)**: 재고 미해제 발견 시 새 해제 로직을 만들지 않고 기존 OrderTerminated를 활성 TX 내 재발행(cancelOne 재호출은 PAYMENT_EXPIRED 멱등 skip이라 직접 publish). AFTER_COMMIT이라 재시도 결과를 같은 회차에 알 수 없어 이번 회차 삭제 이연·다음 배치 재확인. 재고 해제를 서비스 직접 호출로 우회하면 원칙 3(재고는 OrderTerminated만 구독)을 깨므로 기각.
- **채택 — hard delete·soft delete 미도입**: 미결제는 실거래 아닌 가비지. soft delete는 무한 적재라 부적합. 자식 FK 전부 RESTRICT·cascade 없음 → 자식→부모 순차 삭제(payment→snapshot→order_item→order).
- **채택 — 자식 삭제 벌크 DELETE 통일**: reserved 검사용 로드한 managed OrderItem이 부모 em.remove flush 순서와 충돌(TransientObjectException) → 4개 전부 @Modifying 벌크 DELETE로 정합화.
- **채택 — 유예 updated_at+7일**: PAYMENT_EXPIRED는 종결 상태라 updated_at≒종료 시각(불변식: 종료 후 updated_at 갱신 로직 금지). expired_at 전용 컬럼 신설은 비용 대비 효과 낮아 미도입(YAGNI).
- **기각 — 재시도 제거(skip만)**: 재고 미해제 시 영구 점유를 운영자 수동에만 의존하게 됨. OrderTerminated 재발행은 원칙 위반 없이 자동 복구를 주므로 재시도 유지가 낫다(재점검 결론).

### §2 불변식
1. PAYMENT_EXPIRED Order는 읽기 외 모든 비즈니스 상태 전이·자식 생성 대상에서 제외된다(initiate 가드·markPaid 가드·손자 생성 3종 차단으로 코드 성립).
2. PAYMENT_EXPIRED 진입 후 updated_at을 변경하는 비즈니스 로직을 추가하지 않는다(updated_at≒종료 시각·삭제 시점 계산 근거).
3. PAYMENT_EXPIRED 주문은 Inventory 재reserve 대상이 되지 않는다.
4. 삭제 배치는 삭제 가능 조건을 만족한 Order만 삭제한다.

### §3 배선 실측
- initiate 가드: PaymentService.initiate 본인검증 직후 status!=PENDING_PAYMENT → OrderNotPendingPaymentException(422·code ORDER_NOT_PENDING_PAYMENT). GlobalExceptionHandler 매핑.
- 조회: OrderRepository.findByStatusAndUpdatedAtLessThanEqualOrderByUpdatedAtAsc(페이징).
- 삭제: Payment·OrderShippingSnapshot·OrderItem deleteByOrderId(@Modifying JPQL)·OrderRepository.deleteByIdBulk.
- 서비스: ExpiredOrderCleanupService.cleanupOne(건별 @Transactional·(0)status 재확인 (1)PENDING skip (2)reserved==0 확인→순차 삭제 / reserved>0→OrderTerminated 재발행+이연). RESTRICT 위반→계측+ERROR+rethrow(롤백).
- 스케줄러: ExpiredOrderCleanupScheduler(fixedDelay 24h·GRACE_DAYS=7·BATCH_SIZE=100·@ConditionalOnProperty zslab.order.expired-cleanup.enabled·무TX·건별 격리·OrderAutoCancelScheduler 미러).
- 지표: ExpiredOrderCleanupMetrics(zslab.order.expired.deletion_failed{reason}·reserved_unreleased·restrict_violation).
- Payment.fail occurredAt 제거(FE-12 재점검): PaymentFailed 발행 제거로 미사용 파라미터화 → fail(failureCode) 단일 인자.

### §8 carry-over·한계
- "미처리 이벤트 없음" 검사는 outbox/failed_event 테이블 부재로 reserved==0 갈음. OrderTerminated 소비처가 Inventory 단독인 현재만 "이벤트 처리 완료"와 동치. 신규 소비처(알림·통계·정산) 추가 시 reserved==0가 그들의 처리 완료를 보증 못 하므로 삭제 가능 조건 재검토 필요.
- OrderTerminated 재발행은 며칠 전 종료 주문에 "지금 종료" 이벤트를 재발행하는 의미 부조화가 있으나 소비처 Inventory 단독·멱등이라 현재 무해. 소비처 추가 시 재검토.
- idempotency_key 정리는 본 배치 범위 밖(COMPLETED 키 dangling 무해·72h TTL 일반 정리는 별건).
...
## D-179. 관리자 정산 BE — 수수료율 3단 판정·주문 스냅샷·품목 정산·지급예정일·재생성·정산 SMS (Track 85)

날짜: 2026-09-18
범위: Track 85 BE(FE 제외) · V29·V30 · 관리자 정산 API(생성·재생성·확정·지급·조회 4종) · 셀러 정산 조회 3종 · 체크아웃 수수료율 스냅샷 · 정산 확정 SMS
브랜치: feat/track-85-admin-settlement

### 배경
정산 정론 조사 결과를 반영한다. 수수료는 **카테고리 기본율 + 셀러 개별 협의율**이 업계 통상이고, 지급기한은 대규모유통업법상 월 판매마감 후 40일이며 온라인 중개는 20일로 단축(2026-09-17 국회 통과·공포 후 1년 시행) → 지급예정일 기본 오프셋 20일. 정찰(docs/track-85/recon-report.md·STEP 364)에서 확인한 현행: 수수료율은 `seller.commission_rate NOT NULL DEFAULT 1000` 단일, 정산은 셀러 합계 2쿼리(gross·refund)로 헤더만 생성하고 품목 근거가 없으며, 계좌 없는 셀러는 생성에서 skip, 지급예정일·재생성·정산 SMS·조회 API(관리자 목록/상세·셀러)가 없었다. `Settlement.bankAccountId`·`commissionRate`는 생성 시점 스냅샷(`updatable=false`).

### §1-A 결정
1. **수수료율 3단 판정: α 셀러 단일율 유지 【기각】 / β 셀러 개별율 → 카테고리율 → 플랫폼 기본율 【채택】** — α는 정론이 아니며 품목(줄) 단위 율 표시가 불가능하다. `CommissionRateResolver`(settlement/service/CommissionRateResolver.java:66 `resolve(sellerId, categoryId)`·:71 `resolveAll(Collection<CommissionRateKey>)`·셀러·카테고리 각 배치 1회·:99 `resolveOne`). 플랫폼 기본율은 `settlement.default-commission-rate`(application.yml:87·`SETTLEMENT_DEFAULT_COMMISSION_RATE`·기본 1000 bp·V30 백필 상수와 동일해야 함). 카테고리 경로는 `order_item.product_id → product.category_id`(현행·주문 시점 카테고리 이력 컬럼 없음).
2. **주문 시점 `order_item.commission_rate` 스냅샷, 정산은 스냅샷만 사용** — V30 `order_item.commission_rate INT NOT NULL`(백필 `COALESCE(seller, 현행 category, 1000)` 후 NOT NULL·V30:19-31). `OrderItem.commissionRate`(order/entity/OrderItem.java:75·`updatable=false`)·`OrderItemCommand` +commissionRate. `CheckoutService.resolveCommissionRates`(checkout/service/CheckoutService.java:291·양 해소 경로 :231·:270에서 배치 1회). 사후 셀러·카테고리 율 변경은 기존 주문·정산에 무영향이며 재생성(결정 9)도 스냅샷 기준이다.
3. **`seller.commission_rate` NULL 허용·기본값 1000 행은 NULL 환원("개별 계약 없음")** / α "개별 계약 여부" 플래그 컬럼 추가 【기각】 — NULL 자체가 "개별 계약 없음"을 뜻하므로 플래그는 중복. V30 `seller.commission_rate INT NULL`(DEFAULT 제거·`UPDATE seller SET commission_rate = NULL WHERE commission_rate = 1000`·V30:34-38) · `Seller.commissionRate Integer`(seller/entity/Seller.java:61) · V30 `category.commission_rate INT NULL`(V30:15·`Category.commissionRate` category/entity/Category.java:52). 로컬 적용: seller 2건 NULL·category 1건 NULL·order_item 11행 1000.
4. **품목별 floor 합 = fee(합계 버림 아님)** — `SettlementItem.calculateFee(amount, commissionRate)`(settlement/entity/SettlementItem.java:86·`floor(amount × rate / 10000)`)를 품목마다 적용해 합산(SettlementCreationService.java:204). 기존 합계 버림과 값이 달라지며(테스트: 품목별 합 2500 ≠ 합계 버림) 차이는 셀러에 유리한 방향이다. 기존 셀러 합계 2쿼리(`aggregateGrossBySeller`·`aggregateRefundBySeller`)는 대조 테스트용으로 유지.
5. **`settlement_item` 스냅샷(SALE·REFUND)** — V29 `settlement_item`(15컬럼+created_by·`SettlementItemType` enum·`@Enumerated(STRING)`). 중복 방지는 **STORED generated `dedup_key`**(`CONCAT(settlement_id, ':', item_type, ':', order_item_id, ':', COALESCE(refund_id, 0))`·V29:37) UNIQUE `uk_settlement_item_dedup`(V29:40) — SALE 행은 `refund_id` NULL이라 일반 복합 UNIQUE가 NULL≠NULL로 무효(V13 `category.dedup_key` 선례). `fk_settlement_item_settlement`는 **ON UPDATE RESTRICT**(V29:43·generated 컬럼이 참조하는 FK는 CASCADE 불가). 소스 조회는 `OrderItemRepository.findSettlementSaleSources`(order/repository/OrderItemRepository.java:89)·`RefundRepository.findSettlementRefundSources`(refund/repository/RefundRepository.java:124). 헤더 `settlement.commission_rate`는 NULL 허용으로 전환(V29:16·신규 행 NULL·DROP은 §8).
6. **지급예정일 = 기간 말일 + `settlement.payout-offset-days`(기본 20)** — V29 `settlement.scheduled_pay_date DATE NULL`(V29:48). `SettlementCreationService` 생성자 `@Value("${settlement.payout-offset-days:20}")`(SettlementCreationService.java:78)·`Settlement.scheduledPayDate`(settlement/entity/Settlement.java:87·`updatable=false`).
7. **계좌: 생성 조건에서 제외(기존 skip 삭제), 지급 시 필수·그 시점 주 계좌 스냅샷** — 정찰에서 셀러 계좌 등록 경로가 없음을 확인(§2). V29 `settlement.bank_account_id BIGINT NULL`(V29:49). `Settlement.create` 7인자(Settlement.java:95·bankAccountId 없음)·`markPaid(paidAt, bankAccountId)`(Settlement.java:146). `SettlementTransitionService.pay`(settlement/service/SettlementTransitionService.java:98): `canTransitionTo` 선검사 422 → net 음수 422(결정 8) → `findPrimaryBankAccountIds`(seller/repository/SellerBankAccountRepository.java:24·`ORDER BY ba.id`) 없음 → `SettlementBankAccountMissingException` 422 `SETTLEMENT_BANK_ACCOUNT_MISSING`(GlobalExceptionHandler.java:544) → 2건 이상이면 WARN(SettlementTransitionService.java:123·id 최소 건 사용) → `markPaid(now, primaryAccountIds.get(0))`(:129). **STL-3 의미 변경: 정산(생성) 시점 → 지급 시점 계좌 스냅샷.** 셀러 `existsById` skip은 유지(SettlementCreationService.java:123).
8. **net 음수: 정상처리(confirm) 허용·지급 422** — `SettlementNegativeNetException` 422 `SETTLEMENT_NET_NEGATIVE`(SettlementTransitionService.java:112·GlobalExceptionHandler.java:536). 차감 이월 자동화는 §8.
9. **재생성: PENDING만, 비관적 락, 사유 필수, 감사 DELETE+CREATE** / α 수동 가감(조정 항목) 【기각·이월】 — 검수 문제는 "원천 수정 후 재집계"로 대응하고 조정 항목은 별도 정책·화면이 필요해 §8. `SettlementCreationService.regenerate(settlementId, reason, auditContext)`(SettlementCreationService.java:147): `findByIdForUpdate`(SettlementRepository.java:38) → PENDING 아니면 `SettlementInvalidStateException` 422(:150) → `deleteBySettlementId`(:160) → 헤더 delete+flush → 감사 DELETE(reason·:163) → 같은 seller·기간 `collectSources(periodStart, periodEnd, sellerId)`(:177) 재집계 → 대상 없으면 삭제만(`SettlementRegenerateResult.deletedOnly`). `POST /api/v1/admin/settlements/{id}/regenerate`(AdminSettlementController.java:79·`RegenerateSettlementRequest.reason @NotBlank @Size(max = 200)`).
10. **confirm SMS: AFTER_COMMIT 이벤트 + REQUIRES_NEW·수신처 3단 fallback·실패해도 전이 유지** — `SettlementTransitionService.confirm`(:62)이 감사 후 `TracedEventPublisher.publishEvent(SettlementConfirmed)`(:80) → `NotificationSettlementConfirmedHandler`(notification/handler/NotificationSettlementConfirmedHandler.java:32-34·`@TransactionalEventListener(AFTER_COMMIT)` + `REQUIRES_NEW`·NotificationClaimRejectedHandler 동형) → `NotificationService.recordSettlementConfirmed`(NotificationService.java:428). 수신처 `seller.contact_phone` → `SellerUserRepository.findUserIdsBySellerIdAndRoleCode(sellerId, SELLER_OWNER)`(SellerUserRepository.java:31) user.phone → 없으면 skip+WARN(NotificationService.java:450-460). 템플릿 `TPL_SETTLEMENT_CONFIRMED`(NotificationTemplateCodes.java:24)·target SETTLEMENT·본문 "[zslab-mall] yyyy년 M월 정산이 확정되었습니다. 정산금액 N원, 지급예정일 M월 d일.". 발송 FAILED는 `notification_log` FAILED 기록·CONFIRMED 유지.
11. **조회 API: 관리자 4종·셀러 3종, 셀러는 CONFIRMED·PAID·본인만·그 외 404 통일** — 관리자 `AdminSettlementQueryController`(settlement/controller/AdminSettlementQueryController.java:31 `GET /api/v1/admin/settlements?year&month(필수·누락 400)&status&keyword&page&size`·:43 `/{id}`·:49 `/{id}/items?type`·:59 `/api/v1/admin/sellers/{sellerPublicId}/settlements`). `AdminSettlementQueryService`: Specification(period·status·sellerIdIn)·keyword는 `SellerRepository.findIdsByCompanyNameLike`(결정 14)·enrich 3쿼리(seller `findByIdIn`·`findSellerIdsHavingPrimary`·`countByTypeGrouped`)·`totals`는 `sumByStatusForPeriod`(SettlementRepository.java:45·**월 전체·필터 무관**)·상세 계좌는 스냅샷 우선 → 현재 주계좌·끝 4자리·연락처 `PhoneMasker`+`maskEmail`. 셀러 `SellerSettlementController`(settlement/controller/SellerSettlementController.java:35 `GET /api/v1/seller/settlements`·:45 `/{id}`·:52 `/{id}/items`·`SellerActorResolver`)·`SellerSettlementQueryService.SELLER_VISIBLE_STATUSES = {CONFIRMED, PAID}`(SellerSettlementQueryService.java:30)·`findByIdAndSellerIdAndStatusIn`(SettlementRepository.java:55)·PENDING·타 셀러·미존재 모두 `SettlementNotFoundException` 404(존재 은닉·ClaimService 소유 검증 패턴). 정산 식별자는 내부 Long 유지(D-134·셀러 이력 경로만 `slr_` publicId).
12. **환불 집계 D-168 가드 유지·환불 시 수수료 환급 없음(현행 유지)** — `findSettlementRefundSources`는 `oi.confirmedAt IS NOT NULL`(RefundRepository.java:109)로 gross에 집계된 품목의 환불만 차감(이중 차감 없음·§2). REFUND 품목의 fee는 0. 환급 정책은 §8.
13. **수수료율 범위 0~10000 bp 검증: 설정 기본값은 기동 시, 저장값은 판정 시, order_item은 생성 시** — `CommissionRateResolver.requireInRange(rate, source)`(CommissionRateResolver.java:53·`MIN_COMMISSION_RATE = 0`·`MAX_COMMISSION_RATE = 10_000`·:33-34). 생성자(:45)에서 설정값 fail-fast, `resolveOne`(:99)에서 셀러·카테고리 저장값, `OrderItem.create`(OrderItem.java:142)에서 스냅샷 입력. 범위 밖은 `IllegalArgumentException`(출처 포함). Flyway CHECK 제약은 편집 API 트랙에서 함께 판단(§8).
14. **셀러 상호 LIKE에 ESCAPE 절 명시** — `SellerRepository.findIdsByCompanyNameLike`(SellerRepository.java:25·`LIKE :pattern ESCAPE '\\'`·ProductRepository 선례). 호출부 `toLikePattern`이 `\`·`%`·`_`를 이스케이프하고 `%…%`로 감싼다. keyword `%`·`_`는 전체가 아니라 상호에 해당 문자를 가진 셀러만 매칭.
15. **생성 감사 CREATE 추가(기존 confirm·pay UPDATE 유지)** — `createMonthlySettlements(year, month, auditContext)`(SettlementCreationService.java:103)·`createOne` 건별 `AuditLogAction.CREATE SETTLEMENT`(:235·금액 스냅샷 after). `POST /api/v1/admin/settlements`(AdminSettlementController.java:66)에 `HttpServletRequest` 감사 컨텍스트. 재생성은 DELETE(reason)+CREATE(결정 9).

- **멱등·동시성**: 생성 전 `existsBySellerIdAndPeriodStartAndPeriodEnd` seller별 skip 유지, 선확인 통과 후 UNIQUE 위반은 `SettlementAlreadyExistsException` 409로 배치 롤백(SettlementCreationService.java:53-54·현행 유지). 헤더 `saveAndFlush`(:214) 후 품목 `saveAll`(settlement_id FK 필요).
- **`SettlementLineResponse`**(생성 응답): commissionRate 제외·settlementId·scheduledPayDate 추가.
- **`MissingServletRequestParameterException` → 400 MALFORMED_REQUEST**(GlobalExceptionHandler.java:168-171·year/month 누락).

### §2 결정 라운드 재진입
- 정찰에서 환불 이중 차감 없음 확인(D-168 가드 `confirmedAt IS NOT NULL`) → 사양의 "선행 수정" 불필요·가드 유지로 확정(결정 12).
- 정찰에서 셀러 계좌 등록 경로 부재 확인(`seller_bank_account` 쓰기 API 없음) → 사양의 "계좌 없는 셀러 skip 유지"에서 "계좌를 생성 조건에서 제외·지급 시 필수"(결정 7)로 선회. STL-3 의미 변경 명시.
...
- 신규 설정 키 `settlement.default-commission-rate`(`SETTLEMENT_DEFAULT_COMMISSION_RATE`)·`settlement.payout-offset-days`(`SETTLEMENT_PAYOUT_OFFSET_DAYS`)·.env.example 동기화.

### 변경 파일
- Flyway 신규: db/migration/V29__settlement_items_and_payout.sql(settlement_item·dedup_key·scheduled_pay_date·bank_account_id/commission_rate NULL·롤백 주석) · V30__commission_rate_snapshot.sql(category/order_item commission_rate·백필·seller NULL 환원·롤백 역순 주석)
- main 신규: settlement/entity/SettlementItem · settlement/enums/SettlementItemType · settlement/service/CommissionRateResolver·AdminSettlementQueryService·SellerSettlementQueryService·SettlementRegenerateResult · settlement/event/SettlementConfirmed · settlement/exception/SettlementBankAccountMissingException·SettlementNegativeNetException · settlement/repository/SettlementItemRepository·SettlementItemCountProjection·SettlementStatusTotalProjection·SettlementSpecifications · settlement/controller/AdminSettlementQueryController·SellerSettlementController · settlement/controller/request/RegenerateSettlementRequest · settlement/controller/response 11(AdminSettlementListResponse·AdminSettlementSummaryResponse·AdminSettlementDetailResponse·SettlementMonthlyTotals·SettlementSellerRef·SettlementSellerContactResponse·SettlementBankAccountResponse·SettlementItemResponse·SettlementRegenerateResponse·SellerSettlementSummaryResponse·SellerSettlementDetailResponse) · order/repository/SettlementSaleSourceProjection · refund/repository/SettlementRefundSourceProjection · notification/handler/NotificationSettlementConfirmedHandler
- main 수정: settlement/entity/Settlement · settlement/service/SettlementCreationService(재작성)·SettlementTransitionService · settlement/repository/SettlementRepository(JpaSpecificationExecutor+4쿼리) · settlement/controller/AdminSettlementController·response/SettlementLineResponse · order/entity/OrderItem·command/OrderItemCommand·service/OrderService·repository/OrderItemRepository · checkout/service/CheckoutService · refund/repository/RefundRepository · seller/entity/Seller·repository/SellerRepository·SellerBankAccountRepository·SellerUserRepository · category/entity/Category · notification/service/NotificationService·template/NotificationTemplateCodes · common/web/GlobalExceptionHandler · resources/application.yml · .env.example
- test 신규: settlement/migration/V30CommissionRateBackfillMigrationTest(1) · settlement/service/CommissionRateResolverTest(6) · settlement/controller/AdminSettlementQueryControllerIntegrationTest(6)·SellerSettlementControllerIntegrationTest(4)·SettlementConfirmSmsIntegrationTest(4) / test 재작성·수정: settlement/service/SettlementCreationServiceTest(4→6)·SettlementTransitionServiceTest(7→9) · settlement/entity/SettlementTest(6→7) · settlement/repository/SettlementRepositoryTest · settlement/controller/AdminSettlementControllerIntegrationTest(5→8) · checkout/service/CheckoutServiceTest · claim ClaimExchangeIntegrationTest T10 · order/entity/OrderItemTest(+1) · 픽스처: `OrderItem.create`/`OrderItemCommand` 호출 17파일 28곳·raw `INSERT INTO order_item` 40파일 +commission_rate 1000
- 의미 변경 보고: SettlementCreationServiceTest '계좌부재 skip' 케이스 → '계좌 없이 생성'으로 전환(결정 7).
- FE 변경 없음 · 신규 라이브러리 없음.

### 검증
- V30CommissionRateBackfillMigrationTest: V29 → 시드 → V30 백필(COALESCE 셀러/카테고리/1000)·NOT NULL·seller 1000→NULL/500 유지·DEFAULT 제거·category 컬럼.
- CommissionRateResolverTest 6: 셀러율 우선 / NULL→카테고리 / 둘 다 NULL→기본값 / 미존재·배치 1회 / 셀러율 -1·카테고리율 10001 판정 시 거부 / 기본값 -1·10001 기동 시 거부·경계 0·10000 허용.
- SettlementCreationServiceTest 6: 품목 스냅샷·품목별 버림 합(2500 ≠ 합계 버림)·계좌 없이 생성·지급예정일·D-168 가드·aggregate 대조·CREATE 감사 4 / 재생성 성공·삭제만·422·404. SettlementTransitionServiceTest 9: 이벤트 발행·계좌 스냅샷·계좌 없음 422·음수 422.
- AdminSettlementControllerIntegrationTest 8: 생성 품목 2건·CREATE 감사·regenerate 200/422/400. AdminSettlementQueryControllerIntegrationTest 6: 월 필수 400·status/keyword·keyword `%`·`_` 리터럴 매칭·합계·상세 마스킹/계좌 스냅샷/끝4자리·품목 type/페이징·셀러 이력 404·403. SellerSettlementControllerIntegrationTest 4: CONFIRMED/PAID만·PENDING/타 셀러/미존재 404·BUYER 403·미매핑 401. SettlementConfirmSmsIntegrationTest 4: contact_phone/OWNER phone/없음 skip/발송 실패에도 CONFIRMED·notification_log SENT/FAILED.
- 전체 `./backend/gradlew.bat test --rerun-tasks`: 201파일 1084 tests·0 fail·skip 0(1053 → 1084).
- 로컬(docker restart·Flyway validated 30·`SHOW COLUMNS` 확인·ADMIN 토큰으로 `GET /api/v1/admin/settlements?year=2026&month=8` 200·keyword `%`·`_`·`\` 200·기동 ERROR 0).
- 외부 검토: A / 지적 10건 중 수용 3건(수수료율 범위·주 계좌 결정성·LIKE ESCAPE), 문제 없음 6건, 기각·이월 1건(이벤트 실패 지표).

### 외부 검토 반영 (2026-09-18)
- **결정 13 신설 — 수수료율 범위 검증**: `requireInRange` 3지점(기동·판정·생성). 테스트 +4(CommissionRateResolverTest 3·OrderItemTest 1).
- **결정 7 보충 — 주 계좌 결정성**: `seller_bank_account.is_primary`에 UNIQUE·인덱스 없음을 V1~V30 전수 확인 → `findPrimaryBankAccountIds` Javadoc에 `ORDER BY id` 결정성 명시·지급 시 2건 이상 WARN 1줄. 스키마 무변경(§8).
- **결정 14 신설 — LIKE ESCAPE**: `ESCAPE '\\'` 명시·AdminSettlementQueryControllerIntegrationTest Q3에 셀러 상호 `정산셀러B%_` 시드 + keyword `%`·`_` 각 1건 단언.
- 테스트 수 1080 → 1084.

### §8 이월
- **`seller_bank_account.is_primary` 단일 보장 제약** — 현재 DB 무제약·코드에서 `ORDER BY id` 결정성 + 2건 이상 WARN으로 방어. 계좌 등록 API 도입 시 부분 UNIQUE(또는 generated 컬럼 UNIQUE) 함께 도입.
- **셀러·카테고리 수수료율 편집 API·화면** — 도입 시 율 범위 `CHECK (commission_rate BETWEEN 0 AND 10000)` 제약을 함께 판단.
- `settlement.commission_rate` DROP(V29에서 NULL 허용만·신규 행 NULL).
- 환불 시 수수료 환급 정책(현재 REFUND 품목 fee 0·환급 없음).
- 조정 항목(수동 가감)·net 음수 차감 이월 자동화.
...
## D-187. 셀러 상태 관리와 구매 차단 (Track 89-D)

날짜: 2026-09-19
범위: Track 89-D BE · `GET /api/v1/admin/sellers/page`·`GET /api/v1/admin/sellers/{slr_}` · `PATCH /api/v1/admin/sellers/{slr_}/status` · `PUT /api/v1/admin/sellers/{slr_}` · 입점 요청 `ownerUserPublicId` 전환·사업자번호 중복 409 · `SellerStatus.canTransitionTo`·`Seller.changeStatus`·`update` · `SellerTerminationGuard`(G1~G3) · 담기·주문·재결제 판매자 ACTIVE 선검사 · 비-ACTIVE 셀러 관리자 상품 등록 422
브랜치: feat/admin-sellers
정찰: docs/track-89d/recon-report.md(gitignore·로컬)

### 배경 — 정찰 A-6 결론
`SellerStatus`를 읽는 main 코드는 3파일 4지점뿐이었다. **막는 것**: 구매자 카탈로그 목록·검색(`ProductRepository.findDisplayable` `s.status = ACTIVE`)·상세(`ProductCatalogService.getProduct` 404 은닉)·장바구니 조회 `purchasable=false` 플래그(FE 결제 버튼 차단만). **못 막는 것**: 서버 측 담기·주문·재결제(`ProductPurchasePolicy`에 seller 인자 없음·D-160 §8 이월)·셀러 로그인(`DbRoleAuthorization` seller_user 존재만)·셀러 API 9종·정산 생성(`existsById`만)·관리자 상품 등록·승인·진행 중 주문 후속. 즉 정지·종료는 "안 보이게"였지 "거래를 멈추는" 것이 아니었고, 상태를 바꾸는 코드 경로 자체가 0건(엔티티 mutator 없음·JDBC 시드만)이었다. state-machine.md §7에는 전이 6종·불가역·운영자 권한이 이미 기재돼 있었고 코드와 불일치는 없다(코드가 없었을 뿐).

### §1-A 결정
1. **구매 차단 범위 α 이번 트랙에 최소형 포함 【채택】 / β 이월 유지 【기각: 상태를 바꿔도 거래가 멈추지 않으면 화면이 "정지"라고 거짓말을 함】 / γ `ProductPurchasePolicy` 전면 개편(seller 인자 추가) 【기각: 카탈로그·장바구니·주문·재결제 5소비처 회귀 위험】** — 정책 시그니처는 그대로 두고 바깥에서 선검사한다. 담기 `CartService.assertPurchasable` 선두(`findById` 1회·`CartItemNotPurchasableException` 422 재사용) · 주문 `CheckoutService.assertSellersActive`(해소된 상품의 판매자 `findByIdIn` 배치 1회·직접주문·장바구니 양 경로·`assertSellable`보다 앞) · 재결제 `revalidatePayable` 동일. 차단 코드는 기존 `PRODUCT_NOT_ON_SALE`·`CART_ITEM_NOT_PURCHASABLE` 재사용 — 판매자가 팔 수 없는 상태는 구매자 관점에서 "판매 상태 아님"과 같고 FE 분기 소비처가 없어 `PurchaseBlockReason`·`OrderNotPayableReason`을 늘리지 않는다. soft-delete 판매자(미해소)는 fail-closed 차단.
2. **종료 가드 집합 α G1+G2+G3 【채택】 / β 정산만 【기각: 진행 중 주문·클레임이 처리 주체 없는 고아가 됨】** — `SellerTerminationGuard.evaluate`가 상세 응답(`terminable`·`terminationBlocks[{code,count}]`)과 실제 전이(409 `SELLER_ACTIVITY_IN_PROGRESS` + `blocks`)에서 같은 판정을 쓴다(미리보기 = 결과·IT T4 단언). G1 `settlement.status IN (PENDING, CONFIRMED)` · G2 `order_item.item_status ∉ {CONFIRMED,CANCELLED,RETURNED,EXCHANGED} ∧ order.status ∉ {PAYMENT_EXPIRED, CANCELLED}` · G3 `claim.status IN (REQUESTED, APPROVED)`. 주 계좌 없음·SALE 상품 존재는 가드가 아니라 `warnings`로 응답. **판정 시점(외부 검토 지적 1 확정·2026-09-19)**: 가드는 **전이 요청 시점(가드 쿼리 실행 시점) 기준 판정**이며 이후 유입까지 원자적으로 막지 않는다. 근거 — (a) 셀러 종료는 운영자가 정산 지급·주문 처리를 사전 조율한 뒤 수행하는 행위라 가드는 "살아있는 셀러를 실수로 종료하는 것"을 막는 장치이지 밀리초 단위 경쟁을 막는 장치가 아니다 (b) 주문 생성은 이미 셀러 ACTIVE를 검사(`assertSellersActive`)하므로 경쟁 창은 검사~TX1 커밋 수십 ms (c) 원자 보장에는 주문·클레임·정산 생성이 모두 셀러 락을 공유하는 공통 규약이 필요하며 3도메인 수정과 데드락 순서 검토 비용이 얻는 것보다 크다(§8 이월). Javadoc: `SellerTerminationGuard`(판정 시점)·`AdminSellerCommandService.changeStatus`(락 범위 = 동일 셀러 관리자 명령 직렬화).
3. **G2의 PAYMENT_EXPIRED 트랩 — 주문 상태 조인 필수** — 결제 만료 주문의 품목은 `item_status=ORDERED`로 남는다(품목 전이 없음·정찰 실측 셀러 1에 5건). 품목 상태만 보면 만료 주문이 "진행 중"으로 잡혀 종료를 영구 차단한다. `OrderItemRepository.countInProgressBySellerId`가 `JOIN oi.order o`로 제외하며 IT T4(만료 ORDERED 1건 있어도 G2=2)·T5(만료 ORDERED만 있는 셀러 종료 200)가 경계를 고정한다. PENDING_PAYMENT(만료 전 미결제)는 결제될 수 있어 진행 중으로 센다. 주문 수 집계도 같은 이유로 PENDING_PAYMENT·PAYMENT_EXPIRED를 제외한다.
4. **전이 매트릭스 = state-machine.md §7 그대로**: PENDING→ACTIVE·PENDING→TERMINATED(승인 거부)·ACTIVE⇄SUSPENDED·ACTIVE→TERMINATED·SUSPENDED→TERMINATED. 같은 상태 재요청 422(D-160 §1-A 3 관습). `SellerStatus.canTransitionTo` + `Seller.changeStatus`(IllegalStateException → `SellerInvalidStateException` 422 `SELLER_INVALID_STATE`). 순서: 합법성(422) → 가드(409) — TERMINATED 셀러에 다시 TERMINATED는 가드가 아니라 전이 위반. 4층위: DB ENUM(V1 기존)·enum·DTO `@Pattern("^(ACTIVE|SUSPENDED|TERMINATED)$")`(PENDING은 입점 초기값 전용)·FE 상수(FE-40).
5. **상품 자동 변경 없음** — 카탈로그 목록·상세 쿼리가 셀러 ACTIVE를 조건으로 가져 노출은 즉시 자동 차단·복귀된다(IT T7: SUSPENDED 후 목록 0·상세 404 → ACTIVE 복귀 후 재노출·`product.status` 불변). 상품을 STOPPED로 바꾸면 복귀 시 원복 대상 판별이 필요해지고, 셀러 축과 상품 축의 상태가 섞인다.
6. **TERMINATED = `WithdrawnSeller` INSERT(SLR-6·같은 TX)** — `WithdrawnSeller.create(seller, reason, now + 5년)`. `terminate_reason`=요청 사유·`legal_retention_until`=전자상거래법 거래기록 5년(deletion-policy §3 "5년 등")·`anonymized_at` NULL(비식별화 배치는 D-23 후속·§8). `WithdrawnUser`도 main 참조 0건이라 "선례"는 엔티티 컬럼 정의뿐이며 회원 탈퇴는 `withdrawn_at` 마킹만 한다 — 셀러는 status가 SoT이고 사유·보관 만료를 남길 곳이 아카이브뿐이라 INSERT한다. 아카이브 INSERT 실패 시 상태·감사도 롤백(IT T5-2·`@MockitoSpyBean` 예외 주입).
7. **감사·사유** — 전이 `UPDATE SELLER before{status} after{status, reason}`·reason `@NotBlank @Size(200)`. 정보 수정은 6필드 스냅샷 diff + 수수료율이 실제로 바뀔 때만 사유 필수(400 `MALFORMED_REQUEST`·89-C 카테고리 선례)·무변경 감사 skip·204. 비관적 락 `findByPublicIdForUpdate`(같은 셀러 동시 명령 직렬화).
8. **수수료율 적용 시점 = 주문 생성 시 `order_item.commission_rate` 스냅샷(D-179 결정 2)** — 셀러율 변경은 이후 체크아웃되는 신규 주문에만 적용되고 기존 주문·생성된 정산·PENDING 재생성에는 영향 없음. `Seller.update`가 `CommissionRateResolver.requireInRange`로 저장 전 재검증(DTO `@Min/@Max` 우회 경로 방어·`Category.update` 선례).
9. **입점 요청 `ownerUserId`(BIGINT) → `ownerUserPublicId`(usr_)** — D-186 운영자 부여와 같은 이유(FE는 publicId만 앎)·`findByPublicId` 미존재·soft-delete 404. **사업자번호 중복 409 분리**: 종전에는 seller INSERT가 seller_user `saveAndFlush`에서 함께 flush돼 `uk_seller_business_no` 위반이 같은 catch에서 `SELLER_USER_ALREADY_EXISTS`(409)로 오분류됐다(정찰 §6-1 코드 판단·IT ⑦로 실증). `existsByBusinessNo` 선검사 + catch에서 UK 이름으로 구분 → `SELLER_BUSINESS_NO_DUPLICATE`. 정보 수정도 같은 선검사·flush 변환.
10. **비-ACTIVE 셀러 관리자 상품 등록 422 `SELLER_INVALID_STATE`** — 정지·종료 셀러에 상품을 붙이는 운영자 오조작 차단. 승인·드롭다운은 미변경(노출은 어차피 카탈로그 쿼리가 막음·드롭다운은 기존 상품 필터에 필요).
11. **연락처 마스킹 없음**(회원 상세 선례·사업 연락처·운영자가 연락해야 함) · 계좌는 끝 4자리만(`SettlementBankAccountResponse` 규칙). **탈퇴한 소속 사용자 = 행 유지 + `withdrawnAt` 표기** — seller_user는 탈퇴·삭제 후에도 남기는 것이 D-23 B-d6(감사 추적성)이고, 셀러 2의 유일 owner가 탈퇴 회원인 실측처럼 "로그인 가능한 구성원이 0"임을 화면이 드러내야 한다. soft-delete 회원은 user 필드 전부 null(키 생략).
12. **기존 `GET /admin/sellers` 전량 유지·`/page` 신설** — 소비처 FE 4페이지·E2E 3스펙 무수정(IT T2 응답 형태 단언).

### 변경 파일
- BE 신규: `seller/enums/SellerTerminationBlockCode` · `seller/exception/SellerActivityInProgressException`·`SellerInvalidStateException`·`SellerBusinessNoDuplicateException` · `seller/service/SellerTerminationGuard`·`SellerTerminationBlock`·`AdminSellerQueryService`·`AdminSellerCommandService` · `seller/repository/AdminSellerSpecifications` · `seller/controller/request/AdminSellerStatusChangeRequest`·`AdminSellerUpdateRequest` · `seller/controller/response/AdminSellerSummaryResponse`·`AdminSellerDetailResponse` · `product/repository/SellerProductCountProjection` · `order/repository/SellerSalesSummaryProjection`
- BE 수정: `SellerStatus`(canTransitionTo) · `Seller`(changeStatus·update) · `SellerRepository`(JpaSpecificationExecutor·findByPublicIdForUpdate·existsByBusinessNo) · `SellerUserRepository`(findBySellerId) · `ProductRepository`(countActiveBySellerIdsGroupByStatus) · `OrderItemRepository`(countInProgressBySellerId·summarizeSalesBySellerId) · `ClaimRepository`(countActiveBySellerId) · `SettlementRepository`(countBySellerIdAndStatusIn·sumByStatusForSeller) · `AdminSellerController`(+page·get·changeStatus·update) · `SellerProvisioningRequest`(ownerUserPublicId) · `SellerProvisioningService`(findByPublicId·사업자번호 409) · `AdminProductCommandService`(비-ACTIVE 422) · `CartService`(판매자 선검사) · `CheckoutService`(assertSellersActive·SellerRepository 주입) · `GlobalExceptionHandler`(422 SELLER_INVALID_STATE·409 SELLER_ACTIVITY_IN_PROGRESS+blocks·409 SELLER_BUSINESS_NO_DUPLICATE)
- 테스트: `SellerStatusTest`(3)·`SellerTest`(3) 신규 · `AdminSellerManagementControllerIntegrationTest` 신규 11(T1 권한·T2 목록·T3 상세·T4 가드 미리보기=실제·T4-2 단일 가드·T5 허용 6전이+아카이브·T5-2 롤백·T6 금지·T7 카탈로그 연동·T8 PUT·T9 상품 등록 422) · `AdminSellerControllerIntegrationTest` 6건 식별자 교체 + ⑦ 사업자번호 409 · `CheckoutServiceTest` +4(직접 SUSPENDED·장바구니 TERMINATED·soft-delete·재결제 PENDING) · `CartServiceTest` +2 · `CheckoutIntegrationTest` +3(직접·장바구니 담은 뒤 정지·재결제) · `CartControllerIntegrationTest` +2(판매자 seed 추가)

### 검증
- `./gradlew.bat test --rerun-tasks`: 210파일 1170 tests(1141 → +29·0 fail·0 error·0 skip). 기존 카탈로그(ProductCatalogControllerIntegrationTest)·구매(Checkout/Cart)·정산 테스트 무수정 GREEN — 판매자 선검사로 깨진 기존 케이스는 `CartControllerIntegrationTest` 픽스처의 seller 미시드(FK_CHECKS=0·seller_id=1 참조) 1건뿐이며 seller seed 추가로 해소(테스트 의도 불변).
- 외부 검토: 등급 A / 진행 예정(BE+FE 완료 시점).

### §8 이월
- **셀러 로그인·셀러 API 상태 차단**(정찰 N5~N7): `DbRoleAuthorization` SELLER 판정·`SellerActorResolver`에 `seller.status=ACTIVE` 추가 — 셀러 FE 없어 실사용 0이나 API는 열려 있음. 셀러 트랙에서.
- **[정책 미정] 종료 셀러의 정산 생성**: 남은 확정 매출 지급을 위해 비-ACTIVE·TERMINATED 셀러도 정산이 계속 생성된다(G1이 미지급 정산 셀러의 종료를 막고, 종료 후 확정분은 다음 배치가 흡수). 그러나 **언제까지 생성할지·지급 완료 후 처리(생성 중단·계좌 없는 종료 셀러의 지급 차단 영구화)는 정의되지 않음**.
- 계좌 등록·`is_primary` UNIQUE·AES(89-F) · 관리자 주문 목록 seller 필터(상세→주문 링크 부재) · 비식별화 배치·`legal_retention_until` 만료 처리·엔티티 `companyName·ceoName nullable=false` vs V2 NULL 정합(D-23 후속) · 셀러 구성원 추가·역할 변경 API(현재 입점 시 owner 1명만).

### 외부 검토 반영 (2026-09-19)
- **외부 검토: 등급 A / 지적 8건 중 수용 1건(지적 2), 조사 3건(지적 1·4·5), 기각 4건(지적 3·6·7·8).**
- **지적 2 수용 — 사업자번호 catch 원인 구분**: `AdminSellerCommandService.update`의 `DataIntegrityViolationException` catch가 원인 구분 없이 409로 변환하던 것을 `SellerConstraintViolations.isBusinessNoDuplicate`(seller/service·package-private)로 `uk_seller_business_no` 위반만 409·그 외는 rethrow(GlobalExceptionHandler 기본 500)로 고쳤다. 입점 `SellerProvisioningService`도 같은 헬퍼를 쓴다. 판별은 `getMostSpecificCause().getMessage()`의 UK 이름 포함 여부 — 프로젝트에 constraint 이름을 구분하는 선례가 없고(기존 catch 9곳: 운영자 부여·담기·카테고리 2·멱등키·만료 정리·변형 2·정산 생성 — 각 flush 직전 위반 가능 제약이 1개뿐이라 무조건 변환·이번 트랙 밖·수정 안 함), MariaDB 드라이버 메시지 포맷("Duplicate entry … for key 'uk_…'")에 의존한다(Javadoc 명시·판별 실패 시 500으로 흐르는 fail-closed·오분류 없음·Hibernate `ConstraintViolationException.getConstraintName()`은 드라이버별 null 가능성 때문에 미채택). 단위 `AdminSellerCommandServiceTest` 3(UK→409·FK/원인 없음→DIVE 전파·판별 헬퍼).
- **지적 1 조사 — 종료 동시성 경계(판단안대로 확정·2026-09-19·§1-A 2 보충)**: (1) 주문 생성: `CheckoutService`는 `@Transactional`이 없고(:62) `assertSellersActive`는 품목 해소 단계(TX 밖·auto-commit `findByIdIn`·락 없음)에서 실행된 뒤 `revalidateInventory`(읽기) → `orderService.createOrder`(TX1·INSERT+재고 예약)로 이어진다 → 경쟁 창 = 셀러 SELECT ~ TX1 커밋(쿼리 수 회·수십 ms). (2) 클레임 생성 `ClaimService.request`(:122)·`requestByAdmin`(:261)은 셀러 상태를 보지 않고 셀러 락도 없다(소유 검증은 `sellerId` 비교만). (3) 정산 생성 `SettlementCreationService`(:123)는 `existsById`만(status 무관·soft-delete만 제외)·락 없음. (4) 종료 측 `findByPublicIdForUpdate`는 seller 행만 잠그고 가드 3쿼리(settlement·order_item·claim)는 락 없는 읽기라, 가드 통과 후 커밋 전에 유입된 주문·클레임·정산은 막지 못한다. **판단안**: 주문 생성이 이미 셀러 ACTIVE를 검사하므로 창은 "검사~TX1 커밋" 수십 ms이고 단일 운영자가 수동으로 종료하는 빈도를 감안하면 실현 가능성이 낮다 → **가드의 의미를 "전이 요청 시점(가드 쿼리 실행 시점) 기준 판정"으로 확정·문서화**(Javadoc·§1-A 2 보충). 셀러 락 공통 규약은 §8 이월(미해결). 클레임 생성은 의도된 동작, 정산 생성은 정책 미정으로 §8에 구분 기록.
- **지적 5 조사 — 교환·반품 배송(기각 근거)**: `OrderItemStatus.EXCHANGED`는 코드에서 설정되지 않는다 — `ClaimCompletedHandler`(:49-52)가 EXCHANGE 종결 시 품목을 **DELIVERED로 복귀**시킨다(Track 83 D-177 결정 1·구매확정 타이머 재시작). 교환 종결은 교환 OUTBOUND 배송완료 이벤트(`ExchangeDeliveryCompletedHandler` :42-73) → `ClaimExchangeService.completeExchange`(:145-184) 순서라, 재발송 중 품목은 EXCHANGE_REQUESTED(G2 차단)+클레임 APPROVED(G3 차단), 재발송 완료 후 DELIVERED(G2 차단·구매확정까지)다. 반품 RETURNED는 `ClaimRefundCompletedHandler`(:54) → `ClaimService.markCompleted`(:504-513) → `ClaimCompleted`로 설정되며 환불 완료는 검수 PASS 후, 검수는 회수 확인(회수 배송 완료) 후다 → RETURNED 시점에 회수 배송·검수·환불이 모두 끝나 있다. 회수 중 품목은 RETURN_REQUESTED(G2)+APPROVED(G3). **G2에 배송 가드 추가 불요·기각.** G2 종결 집합의 EXCHANGED는 enum 정합용으로 유지(도달 불가).
- **지적 4 조사 — 멱등 키(기각·이월 1건)**: `assertSellersActive`의 `OrderNotPayableException`은 `idempotentCheckout`의 D-66 catch(:164-168)에 포함돼 IN_PROGRESS row가 삭제되고 동일 키 재시도가 허용된다(고착 없음·단위 `checkout_idempotentKey_sellerSuspended_deletesMark` 추가). COMPLETED 캐시(:174-176)는 저장 응답 재생이며 새 주문을 만들지 않는다(멱등 정의상 정상) — 정지 전 성공 주문의 키 재시도는 200 캐시. IN_PROGRESS+order_id≠null(주문 생성 후 initiate 실패·:177-183)은 `completeWithInitiate`로 결제 시작만 재호출하고 셀러·상품 상태를 재검증하지 않는다 — Track 71 상품 상태도 동일한 기존 동작이며 창은 "주문 생성 ~ initiate 실패 사이 같은 키 재시도"뿐 → §8 이월(재결제 `revalidatePayable`와 같은 재검증을 이 경로에도 적용할지).
- **기각 4건**: 지적 3(에러 코드 재사용) — 검토자도 결함 아님으로 결론·구매자 문구는 "판매중지 또는 품절"로 충분·운영 진단용 구조화 로그 `seller_inactive`는 §8 / 지적 6(상세마다 가드 3쿼리) — N+1이 아니라 상세당 고정 3쿼리이며 미리보기와 실제 전이가 같은 `evaluate`를 공유하는 설계 가치가 더 크다 / 지적 8(page size clamp) — FE가 `ADMIN_SELLER_PAGE_SIZES`로 정규화하고 BE clamp(1~100·기본 20)는 안전한 방향이라 계약 불일치가 아니다 / 지적 7 — 지적이 아니라 정상 확인.
- 검증: `./gradlew.bat test --rerun-tasks` 211파일 1174 tests·0 fail·0 error·0 skip(1170 → +4: AdminSellerCommandServiceTest 3·CheckoutServiceTest +1).

### §8 이월 (외부 검토 보강·구분: 미해결 / 정책 미정 / 의도된 동작)
- **[미해결] 셀러 락 공통 규약**: 주문·클레임·정산 생성이 셀러 행 락(또는 상태 재확인)을 공유하는 규약. 현재는 종료 가드 통과 후 커밋 전 유입 창(수십 ms)이 있으며 단일 인스턴스·단일 운영자 전제에서 수용(§1-A 2 판정 시점). **다중 인스턴스 전환 시 재검토**. 도입 시 데드락 순서(seller → order → inventory)를 함께 정한다.
- **[정책 미정] 종료 셀러의 정산 생성**: 위 §8 첫 묶음 항목 참조(언제까지 생성·지급 완료 후 처리 미정의).
- **[의도된 동작·미해결 아님] 종료 셀러의 클레임 생성**: 종료 전 배송분의 반품·교환을 구매자가 계속 청구할 수 있어야 하므로 클레임 생성은 셀러 상태로 차단하지 않는다(구매자 보호). 셀러 측 처리 주체가 없으면 관리자가 대행한다(`requestByAdmin`·승인/거부/검수 Admin wrapper).
...
## D-189. 셀러 구성원 관리 (Track 89-G)

날짜: 2026-09-19
범위: Track 89-G BE · `POST /api/v1/admin/sellers/{slr_}/members`(기존 회원 XOR 신규 계정) · `DELETE …/members/{usr_}`(사유 본문) · `PATCH …/members/{usr_}/role` · 셀러 상세 `members[].joinedAt` · 입점 요청 `ownerUserPublicId` 선택화 · `AdminMemberProvisioningService`(user 도메인·관리자 계정 생성 + 임시 비밀번호) · 정산 SMS fallback 탈퇴 OWNER 제외
브랜치: feat/seller-members
정찰: docs/track-89g/recon-report.md(gitignore·로컬)

### 배경 — 정찰 결론
셀러 상세에 구성원이 표시되지만 추가·제거·역할 변경 수단이 없었고, 셀러 2는 유일 owner가 탈퇴해 로그인 가능한 구성원 0·복구 경로 0이었다. 정찰 전수: **셀러 판정은 `seller_user` 행 존재 하나로 결정된다** — 로그인(`DbRoleAuthorization` `existsByUserId`)·매 요청 액터 해소(`HeaderSellerActorResolver` `findSellerIdByUserId`·캐시 없음)·셀러 API 16개의 소유 검증(전부 `sellerId` 비교·개인·역할 분기 0) 3단 모두 이 테이블만 본다. `SellerUser`는 4컬럼(seller·userId·roleId)·deleted_at 없음(HARD)·FK 피참조 0·쓰기 경로는 입점 시 OWNER 1행 INSERT뿐(UPDATE·DELETE 0). 역할 3값은 `user_role` 0행·`seller_user.role_id` 4행 전부 OWNER·읽는 코드 2곳(입점 OWNER 고정·정산 SMS fallback)·권한 분기 0. 계정 생성 경로는 셀프 가입·부트스트랩뿐이고 관리자 계정 생성 0(임시 비밀번호는 기존 BUYER 재발급만). 셀러 1(데모 상점)은 구성원 0으로 상품·주문을 정상 운영 중(관리자 주도 흐름은 seller_user를 읽지 않음).

### §1-A 결정
1. **구성원 제거 시 토큰 무효화 α 미사용 【채택】 / β `credentials_changed_at` 갱신(Track 84 장치 재사용) 【기각】** — 리졸버가 매 요청 `seller_user`를 조회하므로 행 삭제 = 다음 요청부터 즉시 401(fail-closed)이고 재로그인도 `existsByUserId` false로 401이다(IT T3가 기발급 토큰으로 증명). `credentials_changed_at`은 user 축(그 계정의 모든 역할 세션 무효화)이라 호출하면 BUYER 겸직 세션까지 강제 로그아웃되는 부작용이 생긴다 → 호출하지 않는다(IT T3: 제거 후 BUYER 토큰 200·`credentials_changed_at` NULL 단언). 코어스 `hasRole("SELLER")`는 토큰 claim 기반이라 통과하지만 셀러 컨트롤러 8개 전부 첫 줄에서 resolve하므로 남는 창은 없다.
2. **역할 구현 범위 α 3값 데이터만 유지·권한 분기 없음 【채택】 / β 역할별 권한 분기 구현 【기각: YAGNI·소비처 0·셀러 FE 부재·Track 90 이월】 / γ 전원 동등·OWNER 고정 【기각: 정산 SMS fallback이 OWNER를 실제로 읽고 "대표자" 표기가 무의미해짐】** — 추가·변경 시 `@Pattern("^SELLER_(OWNER|MANAGER|STAFF)$")`(DTO 층)·`RoleCode.valueOf`·역할별 권한 코드 0. **마지막 OWNER 가드 판정 기준 = 대상이 활성(user 행 존재·withdrawn_at NULL) OWNER이고 그 외 활성 OWNER가 0명이면 제거·강등 409 `SELLER_LAST_OWNER`**. 탈퇴·soft-delete OWNER 행은 기능하는 대표가 아니므로 세지 않고 그 행 자체의 제거도 막지 않는다 — 셀러 2처럼 유일 구성원이 탈퇴자인 셀러는 (a) 새 구성원 추가(탈퇴 행 유지·D-187 §1-A 11) 또는 (b) 탈퇴 행 제거 어느 쪽도 가능(IT T5). "구성원 0 금지"·"OWNER 필수"는 가드가 아니다(셀러 1이 구성원 0으로 정상 운영·기존 데이터 무손상). `LastSuperAdminRevocation`(D-186) 동형·셀러 행 비관적 락(`findByPublicIdForUpdate`) 안에서 읽고 판정해 같은 셀러의 두 제거가 동시에 통과하지 않는다.
3. **미가입자 등록 α BUYER 겸직 기본 【채택】 / β 셀러 전용 계정(BUYER 없음) 【기각】** — 회원 목록·상세·탈퇴·임시 비밀번호 재발급(`requireBuyer`)이 전부 BUYER 회원 기준이고 구매자 FE의 비밀번호 변경 화면이 유일한 변경 경로라, BUYER 없이 만들면 그 계정은 어디서도 관리·변경할 수 없다(셀러 FE 부재 기간의 공백). 셀프 가입과 같은 결과물(User·BUYER role·BuyerProfile SILVER·AUTO)에 비밀번호만 임시 발급으로 대체: 해시 저장 → `requirePasswordChange` → 감사 CREATE USER(최소셋·이메일·휴대폰 제외) → `sendSensitiveSms`(같은 TX·FAILED → 502 `TEMPORARY_PASSWORD_DELIVERY_FAILED`·계정·역할·프로필·seller_user 전체 롤백·Track 84 정책 그대로). 평문은 SMS 원문에만 쓰고 응답·로그·감사에 없다(IT T7-1: 응답에 password 키 없음·notification_log content `****`). 요청은 `userPublicId` XOR `newUser{email,name,phone}`(`@AssertTrue` 400 VALIDATION_FAILED)·newUser 검증은 `SignupRequest` 동일·phone은 SMS 수신처라 필수. **이메일 중복만 409(`existsByEmail`·탈퇴 회원 이메일 포함)** — 휴대폰 중복 검증은 셀프 가입에도 없어(user.phone UK 없음·조회 코드 0) 추가하지 않았다(§8·판단 지점). **계정 생성은 user 도메인 `AdminMemberProvisioningService`** — seller 패키지에 계정 생성 로직을 두지 않으며 운영자 신규 계정(89-E 이월)이 같은 서비스를 재사용한다. `markCredentialsChanged`는 호출하지 않는다(신규 계정·무효화할 기존 토큰 없음).
4. **입점 owner 선택화** — `SellerProvisioningRequest.ownerUserPublicId` `@NotBlank` 제거(null 허용). null이면 seller_user 없이 입점(회사 먼저 등록·구성원은 추가 API로), 값이 있으면 종전대로 SELLER_OWNER 1행. 근거: 셀러 1이 구성원 0으로 실제 정상 운영 중이며 owner가 필요한 곳은 셀러 본인 로그인·SMS fallback뿐. owner 없을 때는 `sellerRepository.flush()`로 seller INSERT를 직접 flush해 사업자번호 레이스를 같은 catch에서 잡는다. 감사 diff `ownerUserId`는 owner가 있을 때만. 기존 IT 7건은 owner 필수를 단언하지 않았고(③ 404는 "지정했는데 미존재") 무수정 GREEN·⑧ 신규.
5. **구성원 추가는 사유 없음·제거·역할 변경은 사유 필수** — D-186 §1-A 5 비대칭 선례(부여 가역·감사 CREATE로 충분 / 회수·권한 변경은 맥락 필요). 제거는 DELETE + `@RequestBody {reason}`(`AdminRoleRevocationRequest` 선례). 감사는 전부 `SELLER` 대상(targetId=seller.id): CREATE after `{userId, roleCode, newUserCreated}` · DELETE before `{userId, roleCode}` after `{reason}` · UPDATE before/after `{userId, roleCode}` + `reason`. `PolymorphicTargetType.SELLER_USER` 신설 안 함(SELLER + diff로 추적 충분).
6. **제거 = hard-delete** — seller_user에 deleted_at이 없고 FK 피참조 0. D-23 B-d6 "탈퇴·삭제 후 행 유지"는 회원 탈퇴가 구성원 행을 자동으로 지우지 않는다는 뜻이지 운영자의 명시 제거를 막는 규칙이 아니다(감사 DELETE + 사유가 이력). 제거 후 재추가 가능(IT T3).
7. **추가 거부 집합** — 회원 미존재·soft-delete 404 `USER_NOT_FOUND` · 탈퇴 회원 409 `MEMBER_ALREADY_WITHDRAWN`(로그인 불가 계정을 붙이는 의미 없음) · 이미 이 셀러 또는 타 셀러 소속 409 `SELLER_USER_ALREADY_EXISTS`(`findByUserId` 선검사·V12 user_id 단독 UK라 두 경우가 같은 제약이며 메시지만 구분·코드 신설 안 함) · 역할 오값 400. 제거·역할 변경 대상이 이 셀러 구성원이 아니면 404 `SELLER_MEMBER_NOT_FOUND`(타 셀러 소속 은닉·계좌 404 선례). 같은 역할 재요청 422 `SELLER_MEMBER_INVALID_STATE`(D-187·D-188 같은 상태 재요청 관습).
8. **구성원 목록 = 셀러 상세 `members` 재사용(별도 목록 API 없음)** — `Member`에 `joinedAt`(seller_user.created_at) 추가(additive). 추가 API의 201 본문도 같은 `Member` 레코드.
9. **정산 SMS fallback 탈퇴 OWNER 제외(부수)** — `NotificationService.resolveSellerSmsRecipient` 순회에 `owner.getWithdrawnAt() == null` 조건 1줄(리포지토리·쿼리 불변). 탈퇴 OWNER 행이 먼저 조회돼도 건너뛰고 활성 OWNER로 발송(SMS5)·활성 OWNER 0명이면 종전대로 skip+warn·로그 없음(SMS6). `AdminSellerMemberController` 신설(89-F 도메인 분리 선례·`AdminSellerController` 무수정).

### 변경 파일
- BE 신규: `seller/controller/AdminSellerMemberController` · `seller/controller/request/AdminSellerMemberAddRequest`·`AdminSellerMemberNewUserRequest`·`AdminSellerMemberRemoveRequest`·`AdminSellerMemberRoleChangeRequest` · `seller/service/AdminSellerMemberCommandService` · `seller/exception/SellerMemberNotFoundException`·`SellerLastOwnerException`·`SellerMemberInvalidStateException` · `user/service/AdminMemberProvisioningService`·`AdminMemberProvisionCommand`
- BE 수정: `SellerUser`(changeRole) · `SellerUserRepository`(findByUserId·findBySellerIdAndUserId) · `AdminSellerDetailResponse.Member`(joinedAt) · `AdminSellerQueryService`(joinedAt) · `SellerProvisioningRequest`(owner 선택) · `SellerProvisioningService`(owner null 분기·flush·감사 조건부) · `NotificationService`(탈퇴 OWNER 제외) · `GlobalExceptionHandler`(404 SELLER_MEMBER_NOT_FOUND·409 SELLER_LAST_OWNER·422 SELLER_MEMBER_INVALID_STATE)
- 테스트: `AdminSellerMemberControllerIntegrationTest` 신규 8(T1 권한 401/403/201·204 · T2 추가 즉시 접근: 기발급 SELLER 토큰 401→200·SELLER 로그인 401→200·응답·상세 joinedAt·감사 · T3 제거 즉시 차단: 기발급 토큰 200→401·BUYER 토큰 유지·credentials_changed_at NULL·감사·재추가·사유 400·404 · T4 추가 거부 6종 · T5 마지막 활성 OWNER 제거·강등 409·탈퇴 OWNER 행 제거 204(셀러 2 형태)·다른 OWNER 추가 후 교체 · T6 역할 변경·감사·422·400·404 · T7-1 미가입자 생성(BUYER role·buyer_profile·hash·변경 강제·SMS 1회·로그 마스킹·응답 비밀번호 없음·감사 USER+SELLER·매핑만으로 셀러 API 200) · T7-2 이메일 중복 409(탈퇴 회원 포함)·XOR 400·형식 400·SMS 실패 502 전체 롤백) · `AdminSellerControllerIntegrationTest` +⑧ owner 없이 201 · `SettlementConfirmSmsIntegrationTest` +SMS5·SMS6
- FE 변경 없음(FE-42 후속).

### 검증
- IT: AdminSellerMemberControllerIntegrationTest 8/8 · AdminSellerControllerIntegrationTest 8/8(기존 7 무수정) · SettlementConfirmSmsIntegrationTest 6/6(기존 4 무수정). 테스트 로그에 임시 비밀번호 평문 없음(`****`만).
- 전체 `./gradlew.bat test --rerun-tasks`: 217파일 1208 tests·0 fail·0 error·0 skip(1197 → +11: AdminSellerMemberControllerIntegrationTest 8·AdminSellerControllerIntegrationTest +1·SettlementConfirmSmsIntegrationTest +2). 기존 셀러 API 16개·입점·알림·89-D·89-F 셀러 테스트 무수정 GREEN.
- 로컬 라이브: `docker restart zslab_mall_backend`(Started 79s·Flyway 33 validated·up to date·ERROR 0)·ADMIN 토큰. 데모 상점(구성원 0)에 기존 회원 추가 201(`joinedAt`) → 재추가 409 `SELLER_USER_ALREADY_EXISTS` → STAFF→OWNER 204 → 같은 역할 422 → 마지막 OWNER 제거·강등 409 `SELLER_LAST_OWNER`. owner 없이 입점 201(PENDING·구성원 0). 신규 계정 추가 201 → DB: user(BUYER role·buyer_profile·hash·password_change_required 1·credentials_changed_at NULL)·notification_log SENT `****` 마스킹·audit CREATE USER 최소셋 + CREATE SELLER newUserCreated=true·컨테이너 로그에 평문 없음(MockSmsSender contentLength만). **즉시 차단 실증**: HS256 직접 서명한 SELLER 토큰으로 `/api/v1/seller/settlements` 200 → 제거 204 → 같은 토큰 즉시 401 → BUYER 토큰 `/users/me` 계속 200 → 재제거 404. 무인증 401·XOR 400. 라이브 트랩: Git Bash에서 `/`로 시작하는 JWT_SECRET을 `openssl -hmac` 인자로 주면 MSYS 경로 변환으로 서명 불일치 → `-mac HMAC -macopt hexkey:` 사용. 잔존(로컬): 셀러 6 "89G 라이브검증 셀러"(PENDING·구성원 0)·user 28 live89g@local.test(BUYER·임시 비밀번호) — 삭제 API 없어 유지. 데모 상점의 검증용 OWNER 행(user 20)은 API로는 마지막 OWNER라 제거 불가 → 로컬 단일 행 SQL DELETE로 원상 복구(구성원 0).
- 외부 검토: 등급 A / 진행 예정(BE+FE 완료 시점·2라운드 분할: T1 인증 경계·가드 → T2 나머지). 검토 전 커밋 금지.

### §8 이월
- **셀러 역할별 권한 분기**(Track 90 셀러 FE와 함께 — 정산 조회·계좌 등 OWNER 한정 후보) · **운영자 신규 계정 생성**(89-E·`AdminMemberProvisioningService` 재사용·SUPER_ADMIN 부여와 함께 A등급 경계) · **셀러 상태(SUSPENDED/TERMINATED)에 따른 셀러 로그인·API 차단**(D-187 §8 유지·리졸버 `findSellerIdByUserId`에 status 조인이 확장 지점).
- **휴대폰 중복 검증**: 셀프 가입·관리자 생성 모두 없음(user.phone UK 없음). 필요 시 두 경로에 함께 도입.
- **회원 탈퇴 시 셀러 소속 경고**(확정 4·FE-42): 회원 상세 응답에 소속 셀러를 싣고 관리자 탈퇴 다이얼로그가 "유일 활성 구성원" 경고를 표시. 차단은 하지 않는다(BL-5 셀프 탈퇴 권리).
- 비-ACTIVE(SUSPENDED/TERMINATED) 셀러에 구성원 추가 허용(제한 없음·요구 없음·언급만). soft-delete 회원 구성원 행은 `userPublicId` null이라 경로로 지정할 수 없어 제거 불가(user soft-delete 경로 자체가 main에 없음·실측 0행).
- seed.py step_master 입점 요청의 `ownerUserId`(89-D 이전 필드) 불일치는 그대로(D-188 §8).

### 회원 상세 셀러 소속 필드 (STEP 498·2026-09-19)
- **`AdminMemberDetailResponse.sellerMembership { sellerPublicId, companyName, roleCode, lastActiveMember }`** — 셀러 구성원일 때만(없으면 NON_NULL로 키 생략). 확정 4(회원 탈퇴 시 셀러 소속은 차단하지 않고 경고만)를 FE가 이행하려면 회원 상세가 소속을 알아야 하는데 FE-42 시점엔 응답에 없었다(FE-42 §1-A 9 제안 → 이번 반영). 셀러 2가 바로 이 경로(유일 대표자를 경고 없이 탈퇴)로 만들어졌다.
- **`lastActiveMember` 판정 기준 = 이 회원이 활성(withdrawn_at NULL)이고 해당 셀러의 활성 구성원(user 존재 ∧ soft-delete 아님 ∧ 미탈퇴) 수가 1(= 본인뿐)** — **역할 무관**. 근거: 셀러 로그인·API 접근은 seller_user 행 존재만으로 결정되고 역할은 권한에 관여하지 않으므로(§1-A 2) "탈퇴하면 이 셀러에 로그인할 수 있는 사람이 없어지는가"는 활성 구성원 수로만 답할 수 있다. 구성원 제거 가드의 "마지막 활성 OWNER"(역할 한정·대표 부재 방지)와는 다른 개념이며 Javadoc에 구분 명시. 이미 탈퇴한 회원은 항상 false.
- 조회 비용: `SellerUserRepository.findByUserId`(UK라 1건) + `SellerRepository.findById`(soft-delete 셀러는 null → 키 생략) + `RoleRepository.findById` + 신규 `countActiveBySellerId`(User theta-join·`deletedAt IS NULL AND withdrawnAt IS NULL`) = **상세 1건당 고정 4쿼리**. **목록 응답에는 싣지 않는다**(페이지당 N회 조회 = N+1). 탈퇴·수정·임시 비밀번호 등 명령 동작은 무변경 — IT (11)이 유일 활성 구성원 탈퇴 204를 단언(차단 없음).
- 테스트: `AdminMemberIntegrationTest` (11) 신규(활성 2명 중 1명 false → 다른 1명 탈퇴 후 true·탈퇴 구성원 필드 있음·false·soft-delete 셀러 구성원·미소속 키 없음·목록 키 없음·상세 키 집합 9 + sellerMembership 키 4·유일 활성 탈퇴 204) · 기존 10건 무수정 GREEN. 전체 `--rerun-tasks` 217파일 **1209**(1208 → +1)·0 fail.
- FE(FE-42 §1-A 9 갱신): `AdminMemberDetail.sellerMembership?`·`withdrawSellerWarning`(admin-member-view)·`AdminConfirmDialog` +`warningLines`/`warningEmphasis`(optional·기존 소비처 무영향)·회원 상세 탈퇴 다이얼로그 warning alert. 문구 — 일반: "이 회원은 {상호} 셀러의 구성원({역할})입니다. 탈퇴해도 셀러 소속은 유지되지만 로그인할 수 없게 됩니다." / 마지막 활성 구성원이면 한 줄 추가·굵게: "탈퇴하면 이 셀러에 로그인할 수 있는 구성원이 없어집니다." 확인 버튼은 그대로 활성. vitest +1(369)·Playwright `admin-members ⑤`에 경고 2줄·굵은 줄·확인 활성 단언 추가(62 passed + admin-shell 5 env skip = 67)·픽셀 12/12 diff 0 · 라이브(`step498-live.mjs`·읽기·mutation 0): 셀러 3 OWNER(seller01) 상세 → 경고 2줄·둘째 줄 굵게·확인 활성(취소만) / buyer06 → 경고 없음 / curl: seller01 `lastActiveMember:true`·t75-seller(탈퇴) `false`·buyer06 키 없음.

### 외부 검토 R1 반영 (2026-09-19·인증 경계·가드)
- **외부 검토 R1: 등급 A / 지적 1건 — 수용(제거·역할 변경 404 코드 통일). 핵심 항목 "문제 없음" 확인**: 토큰 무효화 미사용(리졸버 매 요청 조회로 즉시 차단·`credentials_changed_at`은 user 축이라 BUYER 겸직 세션까지 끊으므로 미호출) · 마지막 활성 OWNER 가드 동시성(셀러 행 비관락 안에서 판정) · 계정 생성 TX(SMS 실패 시 전체 롤백) · 임시 비밀번호 미노출(응답·로그·감사 없음·SMS 원문만).
- **지적 1(경미·non-blocking) — `remove`·`changeRole`이 `requireUser` → `requireMember` 순서라 미존재 계정은 404 `USER_NOT_FOUND`, 타 셀러 소속은 404 `SELLER_MEMBER_NOT_FOUND`로 갈려 계정 존재 여부가 응답 코드로 드러남.** 조사: (1) GlobalExceptionHandler에서 두 예외는 HTTP 404 동일이나 code가 달라 본문으로 구분 가능. (2) 기존 관리자 API 정책 전수 — Track 53/89-E 역할 회수 = user 미존재·역할 미보유 모두 `ROLE_ASSIGNMENT_NOT_FOUND`("존재 여부 비노출" 주석 명시) · Track 84 `requireBuyer` = 미존재·비BUYER 모두 `USER_NOT_FOUND` · 89-F 계좌 = 미존재·타 셀러 계좌 모두 `SELLER_BANK_ACCOUNT_NOT_FOUND`(D-188 결정 10 "존재 은닉") · 셀러 API(D-92 Q3) = 타 셀러 리소스를 미존재와 같은 404 → **"경로 하위 대상의 미존재와 범위 밖은 같은 404 코드"로 일관**. 반면 대상을 탐색해 붙이는 API(89-E 부여·입점 owner·구성원 추가)는 `USER_NOT_FOUND` 404 + 소속/탈퇴 409로 구분하는 것이 일관(부여 동형). (3) 위협 자체는 낮다 — 호출자는 인증된 관리자이고 회원 목록 API로 BUYER 계정 존재를 이미 조회할 수 있으며 신규 계정은 전부 BUYER 겸직(셀러 전용 계정 0). 그러나 이 트랙의 제거·역할 변경만 기존 정책에서 이탈하므로 **통일한다**: `requireMemberUser`(회원 미존재도 `SellerMemberNotFoundException`) → 구성원 목록에 없는 대상(미존재·타 셀러 소속·soft-delete)은 전부 404 `SELLER_MEMBER_NOT_FOUND`. 추가 API는 그대로(대상 탐색·404/409 구분·89-E 부여 동형). IT T3·T6에 미존재 계정 = 타 셀러 계정 동일 응답 단언 추가. FE는 `SELLER_MEMBER_NOT_FOUND` 문구를 이미 갖고 있어 무변경.
- **락 전제 확인(검토자 동시성 판단의 전제)**: `SellerRepository.findByPublicIdForUpdate` = `@Lock(PESSIMISTIC_WRITE)` JPQL → 실 SQL `select … from seller s1_0 where (s1_0.deleted_at IS NULL) and s1_0.public_id=? for update`(IT 로그 54회 실측) · `AdminSellerMemberCommandService`는 클래스 `@Transactional`이라 add/remove/changeRole 각각 첫 줄 락 → 커밋까지 한 트랜잭션 · OWNER 상태(`findBySellerId`·`findByIdIn`)는 락 획득 후 `assertNotLastActiveOwner` 안에서 읽는다(락 전 값 재사용 없음) → 같은 셀러의 두 제거 요청은 직렬화되고 두 번째가 최신 상태로 판정한다. 회원 탈퇴(`withdrawn_at` 갱신)는 셀러 락을 잡지 않으므로 "가드 통과 후 커밋 전 다른 OWNER 탈퇴" 창은 남는다(D-187 §1-A 2 종료 가드와 같은 판정 시점 규약·단일 운영자 전제·§8).
- 검증: `AdminSellerMemberControllerIntegrationTest` 8/8(T3·T6 단언 추가) · 전체 `./gradlew.bat test --rerun-tasks` 217파일 1209 tests·0 fail(수 불변·기존 케이스 내 단언 추가).
...
### 외부 검토 반영(2026-09-22 · 같은 브랜치 · 등급 A · 2라운드)
- **R1 Q7 수용 — 셀러 재판매 fail-closed**: 재판매 허용 조건을 "saleStopSource == SELLER"로 변경. ADMIN뿐 아니라 주체 불명(NULL·불변식이 깨진 행)도 422 PRODUCT_STOPPED_BY_ADMIN. RED: STOPPED·NULL 행에 셀러 SALE 요청 `expected:<422> but was:<200>`(통과해 SALE·NULL이 됨) → 적용 후 GREEN(셀러 IT T16·행 불변·감사 0).
- **R2 Q9 수용 — 판매 관리 조작 시 수정 폼 보존**: FE-57 참조(폼 재마운트 금지·카드 상태만 국소 갱신/재조회).
- **R1 Q3 부분 수용(문서만) — §8 V34 실패 복구 절차**: MariaDB DDL은 암묵 커밋이라 `ADD COLUMN` 성공 후 `UPDATE` 백필이 실패하면 컬럼만 남고 `flyway_schema_history`에 V34 실패 행(success=0)이 기록되어 재기동이 막힌다. 복구: ① `SELECT version, success FROM flyway_schema_history WHERE version='34'`·`information_schema.columns`로 `product.sale_stop_source` 존재 확인 → ② (a) 컬럼 DROP(`ALTER TABLE product DROP COLUMN sale_stop_source`) 후 재적용 또는 (b) 백필 수동 완료(`UPDATE product SET sale_stop_source='ADMIN' WHERE status='STOPPED' AND sale_stop_source IS NULL`) → ③ `flyway repair`(실패 행 제거·(b)는 체크섬 보존을 위해 V34 파일 무수정) → ④ 재기동. 불변식 확인 SQL: `SELECT COUNT(*) FROM product WHERE (status='STOPPED') <> (sale_stop_source IS NOT NULL)` = 0.
...
## D-216: 불일치 기록(P1·P6) — reconciliation_issue · PG 충돌 기록 · 점검 스케줄러 · 관리자 화면 (Track 104-2) (2026-09-23)

배경: 104-2 정찰(`docs/track-104/recon-report-104-2.md`)에서 PG가 이미 끝낸 사실(결제 성공·취소·환불 완료)을 내부 규칙과 충돌한다는 이유로 거부(422·409·404·500)해 트랜잭션째 롤백하는 경로가 15건이었다 — 사실은 로그에만 남고 PG는 재전송하거나 포기했다(P1 위반). 저장된 행끼리 어긋난 상태(환불 없는 결제 취소·전액 환불인데 결제 PAID·클레임 종결인데 품목 미전이 등)는 탐지·표식이 없었고 관리자 화면은 결제 1종(C-12)만 계산형 배지로 보였다(P6 위반).

결정:
- **저장소** `reconciliation_issue`(V35): 유형 11(DB ENUM·Java enum·FE 상수 4층위) · (유형, 중복 키) UNIQUE + `INSERT … ON DUPLICATE KEY UPDATE id = id` 멱등 기록 · 상태 OPEN/RESOLVED · 대상 id는 전부 논리 참조(FK 없음). 기록은 `ReconciliationIssueRecorder` 1곳(호출자 트랜잭션 MANDATORY).
- **PG 충돌 13건 전환**(P-1~P-9·R-1·R-2·R-3·R-6): 상태를 바꾸기 전에 판정 → 불일치 1행 → 상태 불변·정상 종료 · 웹훅 200 · mock 결제 콜백(구매자 화면)은 기록 커밋 뒤 기존 4xx · **매칭 행 없음만 기록 커밋 뒤 기존 422(결제)·404(환불)** 유지 + 같은 종류 통지가 재전송으로 매칭돼 정상 처리되면 그 열린 행을 시스템이 자동 해소.
- **환불 완료 시점 기록**: U7(구매확정 품목 있는 전액 환불)·U8·U9(교환·거부 클레임의 환불) — 기존 전이·저장 그대로.
- **점검 스케줄러**(하루 1회·`RECONCILIATION_CHECK_ENABLED`): U5·U6·U7·U10~U13 7패턴을 (패턴, 대상 id) 커서로 끝까지 조회해 기록만 한다(업무 데이터 무변경).
- **관리자**: 목록(상태·유형 필터)·해결(메모 필수·감사·이미 해결 422)·주문 상세 불일치 섹션·대시보드 처리 대기 9번째 칸. Track 103 C-12 계산형 배지는 섹션으로 대체(수동 취소 버튼 조건은 유지).

### §1-A 갈림길·채택/기각 근거
- **저장 자리 = 새 테이블 【채택: 열림/해결 상태·유형 필터·대상 참조·중복 방지가 한 행에 필요하고, 매칭 행이 없는 통지·클레임·배송 대상까지 담아야 한다】** / audit_log 【기각: 행위 기록이라 상태·중복 방지가 없고 "지금 열린 불일치"를 셀 수 없다】 / 기존 엔티티에 표식 컬럼 【기각: 매칭 행 없는 통지를 담을 곳이 없고 결제·환불·클레임·배송에 흩어져 집계·화면이 테이블마다 갈린다】.
- **충돌 처리 = 불일치 행 + 상태 불변 【채택: 무엇이 옳은 상태인지는 운영자가 PG와 대조해 판단할 일이다. 표식만 남기면 상태기계·Resolver·정산은 그대로다】** / 상태기계에 충돌 전이 추가(예: 결제 CONFLICT) 【기각: 상태 집합이 늘면 Resolver·정산·구매확정 가드·화면 전반이 파급되고, 잘못 고른 전이는 되돌리기 어렵다】.
- **과거 건 = 발생 시점 기록 + 점검 스케줄러 【채택: 스케줄러 첫 실행이 저장된 행으로 판정 가능한 과거 건을 같은 조회로 멱등 기록한다(라이브 실측 U5 1건 = 정찰의 order id=2)】** / 도입 시 백필 스크립트 【기각: 스케줄러와 같은 판정을 한 번 더 구현하는 중복이고, 거부·롤백으로 로그에만 남은 과거 PG 사실은 백필로도 복원되지 않는다】.
- **유형 통합 【채택: 운영자 조치가 같은 묶음은 한 유형 + detail.reason — U1·U2·U3 → PG_PAYMENT_SUCCESS_CONFLICT · U8·U9 → REFUND_ON_INVALID_CLAIM · U10~U13 → ITEM_STATE_DRIFT(reason = 점검 패턴명)】** / 원인별 유형 【기각: DB ENUM·FE 상수·필터만 늘고 조치는 같다】.
- **주문 밖 유형 제외(U15 셀러 행 부재·U16 주 계좌 복수) 【채택: 주문 쓰기 락·주문 상세 섹션 규약 밖이고 정산 트랙(104-3) 소관이다】** / 같은 저장소에 포함 【기각: order_id 없는 비PG 행이 섞여 주문 기준 규약이 흐려진다】.
- **U14 제외**: 교환 배송 완료인데 배송·클레임 행 부재 — FK상 저장된 행으로 도달할 수 없어 점검 패턴이 성립하지 않는다.
- **재고 사전 판정 제외(결제 승인 충돌 판정)**: 재고 확정 실패(INV-3·4)는 동기 핸들러 예외로 롤백을 유지한다 — 재고 규칙을 결제 서비스에 복제하지 않기 위함. 결제 행은 있는데 주문 행 없음·락 없는 재계산도 예외 유지 + "도달 경로 미발견" 주석. 품목 PAID 전이 가능 여부(P-8)·환불 완료 시 결제 취소 가능 여부(R-6)는 전이 전 판정으로 전환했다(도달 경로 미발견 주석 병기).
- **매칭 행 없는 웹훅 = 기록 + 기존 422/404 + 재전송 매칭 시 SYSTEM 자동 해소 【채택: 결제 시작·환불 개시는 PG를 부른 뒤 커밋한다 — 커밋 전에 도착한 통지는 PG 재전송으로만 반영된다(셀프 리뷰 #1·상). 기록은 커밋되고 재전송은 같은 키라 행이 늘지 않는다】** / 200 【기각: 재전송이 끊겨 환불 완료 사실이 영구 누락된다】.
- **자동 해소 조건 = 같은 종류 통지 + 정상 처리 뒤 【채택: 매칭 없음 중복 키에 통지 종류를 넣고(결제 attemptKey + 콜백 유형 · 환불 pgRefundId + 콜백 상태) 상태 전이가 끝난 직후에만 해소한다 — SUCCESS가 매칭 없이 기록된 뒤 CANCEL이 매칭돼 처리되거나, 재전송이 충돌·멱등 NO-OP로 끝나면 SUCCESS 사실은 반영되지 않았다(외부 검토 지적 2·RED 실측)】** / 같은 대상 키 + 주문 락 직후 해소(1차) 【기각: 다른 종류 통지·충돌 기록만으로 열린 불일치가 닫혀 사실이 사라진다】.
- **자동 해소 = 조건부 UPDATE(status = OPEN) 【채택: 관리자 해결과 겹치면 행 락에서 기다린 뒤 조건이 다시 평가돼 0행 — 먼저 커밋된 해결(처리자·메모)을 덮어쓰지 않고, 감사는 실제로 바꾼 1행일 때만 남긴다(외부 검토 지적 3·LockRace RED 실측 `RESOLVED|NULL|자동 해소 메모`)】** / 엔티티를 읽고 고쳐 쓰기(1차) 【기각: 락 없는 읽기가 OPEN을 본 뒤 전 컬럼 UPDATE가 관리자 해결을 덮고 감사가 2건이 된다】.
- **스케줄러 기록 = 주문 쓰기 락 → 같은 조회를 대상 1건으로 재확인 → 기록 【채택: 후보 조회는 락 없는 스냅샷이라 기록 시점에 풀렸을 수 있다. 104-1 규약(첫 DB 접근이 주문 락)과도 맞는다】** / 후보를 그대로 기록 【기각: 이미 풀린 건이 열린 불일치로 남는다】.
- **점검 소진 = (패턴, 대상 id) 커서 페이지 순회 + 실행당 전체 상한 + 인스턴스 필드 커서 이어받기 【채택: 페이지 200건씩 패턴 끝까지 훑고 한 실행은 전 패턴 합계 2000건까지 — 상한에 걸리면 다음 실행이 커서부터, 끝에 도달하면 처음(0)으로(Track 99 D-210 DeliveryAutoComplete 방식). 재확인에서 풀리거나 기록에 실패해 남는 후보가 앞 페이지를 차지해도 커서 뒤 후보가 굶지 않는다(외부 검토 지적 5)】** / 패턴별 상한 200 + 기록된 대상 NOT EXISTS만(1차) 【기각: 기록되지 않고 남는 후보가 200건을 넘으면 매 실행 같은 앞 200건만 보고 뒤 후보는 영영 차례가 오지 않는다】.
- **해결 = 표시만 【채택: 보정은 기존 관리자 조작(수동 결제 취소·클레임 처리)으로 하고 여기엔 확인 메모를 남긴다. 해결 행도 키를 유지해 같은 불일치는 다시 열리지 않는다 — 되돌릴 수 없는 조작으로 FE-64 확인 문구】** / 해결 시 자동 보정 【기각: 유형마다 올바른 보정이 달라 운영자 판단을 대신할 수 없다】.

### §2 확정 구현 규칙·트랩
- 중복 키: `payment:{결제 id}`(결제 충돌·U5·U6·U7 — 환불 완료 시점 U7과 스케줄러 U7이 같은 키) · `refund:{환불 id}`(R-1·R-2·R-6·U8·U9) · `payment-attempt:{attemptKey}:{SUCCESS|FAILURE|CANCEL}`·`pg-refund:{pgRefundId}:{SUCCESS|FAIL}`(매칭 없음·통지 종류 포함 — `ReconciliationIssueRecorder.unmatched*Key` 단일 소스) · `claim-completed:`·`claim-rejected:`·`delivery-shipping:`·`delivery-delivered:` + id(ITEM_STATE_DRIFT 4패턴).
- **예외 없는 정상 종료여야 기록이 커밋된다**: 기록기는 호출자 트랜잭션에 참여하므로 충돌 경로에서 예외를 던지면 기록까지 롤백된다. 4xx를 유지해야 하는 곳(mock 콜백·매칭 없는 웹훅)은 서비스가 유형을 돌려주고 컨트롤러가 서비스 트랜잭션 커밋 뒤에 던진다. UNIQUE 충돌도 예외가 나면 트랜잭션이 롤백 전용이 되므로 `ON DUPLICATE KEY UPDATE id = id`로 무변경 처리한다.
- 판정 순서(결제 승인): PAY-3a(같은 주문 PAID) → 주문 재적재·PENDING_PAYMENT → 품목 PAID 전이 가능 → pgTid 선조회(`existsByPgProviderAndPgTid`) → `complete`. 선조회 뒤 다른 주문의 동시 승인과의 UNIQUE 충돌은 기존 flush 409(롤백)로 남는다(단위 테스트로 고정).
- 시스템 자동 해소는 `resolved_by` NULL(V35 CHECK가 허용) + 메모 "재전송 매칭으로 자동 해소" + 감사 SYSTEM · 응답 `resolvedBySystem`. 호출 위치: 결제는 저장·flush·발행 뒤 결제 상태가 바뀐 경우만, 환불은 `markCompleted` 결과 COMPLETED·`markFailed` 뒤만. 실행은 `resolveBySystemIfOpen`(JPQL 조건부 UPDATE) 1행일 때만 감사. 관리자 해결은 행 락(`findByIdForUpdate`)으로 동시 해결 시 뒤 요청 422.
- **외부 검토 기각 근거 — 지적 1(선조회 뒤 다른 주문의 동시 승인과 pgTid UNIQUE 충돌이 409·롤백으로 기록 없이 끝남)**: 롤백된 통지는 PG가 재전송하고, 재전송은 먼저 커밋된 결제의 pgTid를 사전 판정(`existsByPgProviderAndPgTid`)에서 보아 `PG_TID_CONFLICT`로 기록한다 — 사실은 재전송 경로에서 남는다.
- **외부 검토 기각 근거 — 지적 4(동기 핸들러 실패로 결제·환불 완료 전체 롤백)**: 재고 확정·주문 전이 등 동기 핸들러 실패의 롤백은 의도된 설계다(재고 규칙을 결제 서비스에 복제하지 않음 — §1-A). 5xx·4xx 응답이 PG 재전송을 부르고 원인이 해소된 뒤 재처리된다.
- `application.yml` non_null 직렬화 — 주문 없는 행은 `orderId` 키 자체가 없다(FE 타입 optional·IT는 "키 없음"으로 단언).
- 웹훅 DTO 길이 선검증(`paymentAttemptKey`·`pgRefundId` 100) — 매칭 없는 통지를 기록하므로 초과분이 INSERT 오류(500)로 새지 않게 400(셀프 리뷰 #6).
- **감사 필드 스캐너 트랩**: `AuditFieldMaskingPolicyTest`는 `auditRecorder.record`가 있는 파일의 `.put("키"` 전부를 감사 필드로 모은다 → RefundService의 불일치 세부 키 7개(callbackStatus·claimStatus·claimType·completedTotalAfter·failureReason·paymentAmount·paymentStatus)가 섞였다. 민감정보 없음 → 기록 필드 61 → 68.
- **영향 테스트 선정 트랩(BE 1회차 5 실패)**: 변경 서비스를 `@InjectMocks`로 조립하는 단위 테스트(PaymentEventTest·PaymentIdempotencyTest)가 새 의존(자동 해소 호출)을 공통 경로에서 부르게 돼 NPE — 충돌 경로 테스트만 골라 빠졌다. 이후 규칙: 변경 클래스명을 참조하는 테스트 전부 + 소스 스캔형 정책 테스트는 BE 변경 시 항상 포함(PROGRESS 기록).
- RED(전환 전·저장소만 있는 상태·`PgCallbackReconciliationIntegrationTest`): 15/15 실패 — 422 ×8·409 1·404 1·500 2(롤백 = 사실 소실)·U7·U8·mock 불일치 0행 → GREEN 15/15(+ 결정 1 자동 해소 1 = 16). 스케줄러·관리자 API는 신규 기능이라 의미 있는 RED 없음.
- 기존 거부 단정 테스트 17건 갱신(200/반환 유형 + 불일치 1행 + 상태 불변) · 이후 매칭 없음 2건(P-7·R-3·웹훅 IT 2)은 결정 1로 4xx 단정 복귀.
- 검증(2026-09-23): `gradlew test --rerun-tasks` **1535/0 실패/0 skip**(259 클래스·1회차 5 실패 → 승인 후 2회차) · typecheck 0 · vitest **753**(106 파일) · 컨테이너 재생성 healthy 130s·라이브 V35 적용·점검 1회 U5 1건 · Playwright **112 passed / 1 failed / 2 skipped** → 실패 admin-categories ① 단독 1 passed = 콜드 트랩(LT-22) → 실질 113/0/2.
- 셀프 리뷰(A): 지적 10건(상 1·중 2·하 4·결함 없음 3) → 수용 4(#1 매칭 없음 응답 → 결정 1 · #6 입력 길이 · #9 FE 감사 대상 유형 · #10 테스트 보강) · 이월 4(#2·#3·#4·#5) + #6 키 체계 부분 이월.
- 외부 검토 반영 후 검증(BE만·FE 무변경): RED — 다른 종류(CANCEL)·충돌 재전송이 매칭 없음 행을 RESOLVED로 닫음 2건 · 관리자 해결이 자동 해소에 덮임 1건 → GREEN · 영향 테스트(변경 클래스명 참조 전부 + AuditFieldMaskingPolicyTest) 통과 · `gradlew test --rerun-tasks` **1542/0 실패/0 skip**(261 클래스·+7).

### §8 이월
- **104-3 필수**: 실패 처리한 환불의 PG 성공 사실(`PG_REFUND_SUCCESS_ON_FAILED`)을 환불 금액·재개시 판정에 반영(현재 관리자 재개시의 PAY-1 사전 검사는 완료 환불만 세어 PG 이중 환불이 가능 — 기존 위험) / 완료 환불에 FAIL 통지가 오면 500·무한 재전송 → 불일치로 기록.
- 결제완료 결제의 FAILURE 통지·다른 pgTid의 SUCCESS 통지가 기록 없이 NO-OP다.
- mock 환불 복구 스케줄러가 충돌로 PENDING에 남은 환불(R-1·R-6)을 10분마다 다시 처리한다(기록은 멱등·로그는 "성공"으로 집계·id 오름차순 100건 창을 차지할 수 있음).
- `FULL_REFUND_PAYMENT_NOT_CANCELLED`가 두 상황을 뜻한다 — 환불 완료 시점 결제 취소 불가(`refund:` 키)와 스케줄러의 전액 환불인데 PAID(`payment:` 키).
- (반영됨·외부 검토 지적 2) 매칭 없음 키에 통지 종류를 넣어 같은 시도의 SUCCESS·CANCEL이 각자 남는다.
- 점검 스케줄러는 `fixedDelay`만 있어 재기동마다 즉시 점검을 1회 돈다(멱등이라 행은 늘지 않음). 커서도 인스턴스 필드라 재기동하면 처음부터 다시 훑는다.
