...
| PRD-6 | Product.status 전이 = PENDING→SALE(승인)·PENDING→REJECTED(거부)·REJECTED→PENDING(거부 철회·Track 101-A)·SALE↔STOPPED(판매중지·재판매)만 | 상품 공개 통제(운영자 승인 게이트·판매 상태 전환·오거부 복구) | Domain(ProductStatus.canTransitionTo) + Service(운영자 Track 50/71/101-A·셀러 Track 96-5) | PENDING만 승인/거부 대상·REJECTED만 철회 대상·SALE=카탈로그 노출·REJECTED=미노출·STOPPED=목록 숨김 | — |
...
## 주문·결제·정산 정합성 원칙 (Track 104)
- P1 외부 사실 기록: PG에서 일어난 결제 성공·환불 완료는 내부 규칙과 충돌해도 거부하지 않고 저장한다. 충돌은 무시가 아니라 불일치 표식으로 남긴다.
- P2 정산은 남은 돈만: 정산 대상 여부는 결제 상태가 아니라 품목 단위 순수령액(결제 − 완료 환불)으로 판단한다. 불일치 표식이 있는 주문은 정산에서 자동 보류한다.
- P3 되돌리지 않고 상쇄: 확정·정산된 뒤에 생긴 환불은 확정을 취소하지 않고 정산 차감 항목으로 처리한다.
- P4 파생 상태는 판단 근거 아님: Order.status는 표시용 캐시다. 돈·권한·가드 판단에 쓰지 않으며, 재계산은 최신 커밋 기준으로 한 곳에서 한다.
- P5 주문 단위 직렬화: 주문에 속한 결제·환불·클레임·품목·배송·주문 상태를 바꾸는 트랜잭션은 모두 주문(Order) 행 락을 가장 먼저 잡는다. 락 이후의 판정은 최신 커밋을 본다.
- P6 불일치 가시화: 불일치는 로그로 끝내지 않고 영속 상태로 저장해 관리자 화면에 드러낸다.
