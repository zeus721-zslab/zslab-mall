...
> **버전: v2.1** (v2 → v2.1 외부 검토 흡수·D-72~D-77·코드 SoT 정합 보정)
...
| Flyway | V5__refund_constraints.sql 신규 — idx_refund_payment_status·chk_refund_completed_at·uk_refund_pg_refund_id (CR-13·CR-14·CR-15·D-76) |
...
> **V5 제약 (D-76·CR-13~CR-15)**: `idx_refund_payment_status(payment_id, status)` · `chk_refund_completed_at`: PENDING→refunded_at IS NULL / COMPLETED→refunded_at NOT NULL / FAILED→무조건 허용(D-70: COMPLETED 전이 시각만·미완료 행 제약 없음) · `uk_refund_pg_refund_id` UNIQUE (NULL 다건 허용·initiate 실패 시 NULL 허용)
...
| 메서드 | 입력 | 출력 | 책임 |
|---|---|---|---|
| initiate(claimId, amount) | Long·long | Refund | Claim.APPROVED 검증·PAY-1 사전 누적 검증·payment_id 해소(Claim→OrderItem→Order→PAID Payment)·Refund(PENDING) INSERT·PG refund() 호출·pg_refund_id 수신·저장 후 반환 (D-72·[I1]) |
...
| 항목 | 값 |
|---|---|
| Path | `POST /api/webhooks/refunds` (Track 3 `/api/webhooks/payments` REST 복수형 컨벤션 일치·[I4] B 채택) |
...
| 경계 | 내용 |
|---|---|
| initiate | Refund INSERT + PG refund() 호출 — **단일 @Transactional** (D-73·[I3]). PG 호출 실패는 비즈니스 실패로 취급 → Refund=FAILED 저장 후 정상 커밋. FAILED 저장 전 RuntimeException 재던지지 않음. |
...
- ~~decisions.md D-67~D-71 박제~~ → **완료** (feat/track-5-refund-flow)
- ~~D-72~D-77 박제 (외부 검토 흡수)~~ → **완료** (feat/track-5-refund-flow·본 spec v2.1 동반)
...
| 버전 | 날짜 | 내용 |
|---|---|---|
| v2 | 2026-06-28 | 최초 박제 — 외부 검토 흡수 D-67~D-71 반영 |
| v2.1 | 2026-06-28 | 외부 검토 흡수 D-72~D-77 반영 · 코드 SoT 정합 보정 (refunded_at·COMPLETED·pg_refund_id 발급 시점·TX 경계·webhook URL·V5 제약) |