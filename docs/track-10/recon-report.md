...
> 트랙: Track 10-B Admin Claim endpoint
> 정찰일: 2026-06-30
> 정찰 방식: MCP read-only (Claude Code)·코드/테스트/DDL 변경 0건
> 정찰 commit: main HEAD 1b264f3 (feat/track-10 PR #73 머지 직후·working tree clean)
> 브랜치: main 유지 (정찰 단계·결정 라운드 종료 후 feat/track-10-pr-b 생성)

Track 10-B = **Admin Claim endpoint**. Track 10 (feat/track-10·D-92)에서 `SellerClaimController.approve/reject`·`SellerActorResolver seam`이 신설됨. 본 정찰은 D-87 Q3 "Admin 별도 트랙·~70% Service 재사용" 및 D-92 횡단 원칙("wrapper 진입점에서 캡슐화·Track 10-B 재사용 의무")에 따라 Admin 진입 구조를 실측 데이터 기반으로 박제한다.

**후속 자연 진입 식별자 (정찰 룰 #4)**:
- **D-87 Q3**: "Admin API = 별도 트랙 (Track 9+)·~70% Service 재사용"
- **D-88 Q2 후속**: "Track 10 = Claim Admin API (Seller/Admin 승인·거절 endpoint·권한 매트릭스)"
- **D-92 횡단 원칙**: "Track 10-B·Track 11 재사용 의무·wrapper 진입점 캡슐화"

**가드 2 라벨 잠정**: S급 (D-92 동일·권한 인프라 신설·Spring 라우팅 충돌 해소 포함). 외부 검토 권장.

---

## 11. SoT 정독 결과 (PR-B 정찰 기준·정독 완료 박제)

| SoT | 항목 | PR-B 관련 핵심 |
|---|---|---|
| decisions.md D-87 | Q3 | "Admin API = 별도 트랙·~70% Service 재사용"·Track 9+ |
| decisions.md D-88 | Q2·후속 | "Seller/Admin endpoint Track 10 이연"·"Track 10 = Spring Security 진입 후보" |
| decisions.md D-89 | Q3·Q7·Q8 | ClaimInvalidStateException 422·requestedBy 미노출·소유권 Service 2단계·404 정보 노출 회피 |
| decisions.md D-90 | Q2·백로그 | Refund 자동 트리거 Track 10 이연·ClaimApproved Javadoc 보강 Track 10 동반 |
| decisions.md D-91 | 전건 | 통합 테스트 seed FK 부모 그래프 신설 의무·후속 핸들러 트랙 재발 차단 |
| decisions.md D-92 | 전건 | Track 10 Seller 완료·횡단 원칙·wrapper 캡슐화·WARN-5 테스트 책임 경계·Q8 β Refund 미포함 |
| decisions.md D-39 | 전건 | X-Buyer-Id stub·"Seller/Admin 인증 후속 트랙"·범위 외 명시 |
| decisions.md D-40 | 전건 | URL 액터 중립·`/api/seller/` prefix 금지·Controller 액터별 분리·`@PreAuthorize` 후속 |
| live-traps LT-01~03 | 전건 | LT-01 CHAR public_id·LT-02 FK_CHECKS try-finally·LT-03 @SQLRestriction |
| recon-report.md §1~§9·§4.8·§5.6·§6.5 | 전건 | PR-A 산출물 전건 재정독·D-92 결정 흡수 확인 |

---

## 12. 실측 결과 (STEP 1·2 15파일군·메서드 목록·1줄 인용)

### 12.1 SellerActorResolver.java (전체 read·23줄)

> 실측: `public interface SellerActorResolver` — 메서드 1건: `Long resolve(HttpServletRequest request)`. Javadoc line 9 **"후속 Admin/Buyer 액터 resolver는 본 인터페이스를 공유하지 않고 별도 인터페이스로 분리한다"** 명시. 패키지 `com.zslab.mall.common.auth`. → Admin 전용 AdminActorResolver 별도 인터페이스 신설 의무 실측 확정.

### 12.2 HeaderSellerActorResolver.java (전체 read·31줄)

> 실측: `@Component HeaderSellerActorResolver implements SellerActorResolver`. SELLER_ID_HEADER = "X-Seller-Id". `resolve(HttpServletRequest)`: null/blank → UnauthenticatedException(401)·NumberFormatException → MalformedRequestException(400). D-39 X-Buyer-Id stub 패턴 1:1 정합. → Admin 패턴: HeaderAdminActorResolver implements AdminActorResolver, X-Admin-Id 헤더, 동일 401/400 패턴.

### 12.3 SellerClaimController.java (전체 read·81줄)

> 실측: `@RestController @RequestMapping("/api/v1/claims")`. 2 endpoint: `@PostMapping("/{claimPublicId}/approve")`·`@PostMapping("/{claimPublicId}/reject")`. 4 필드 주입: ClaimService·ClaimRepository·OrderItemRepository·SellerActorResolver. `toResponse(String claimPublicId)` private 메서드(재조회 패턴). **→ AdminClaimController도 동일 base path 사용 시 @PostMapping 충돌 후보(WARN-1·Q6 핵심 의제).**

### 12.4 ClaimService.java (전체 read·283줄)

| 메서드 | 시그니처 | 상태·Admin scope |
|---|---|---|
| `request` | `Claim request(ClaimRequestCommand)` | Buyer scope·변경 없음 |
| `approve` | `void approve(Long claimId, LocalDateTime processedAt)` | **actor 비의존 primitive·D-92 횡단 원칙·Admin 재사용 가능** |
| `reject` | `void reject(Long claimId, LocalDateTime processedAt)` | **actor 비의존 primitive·Admin 재사용 가능** |
| `approveBySeller` | `void approveBySeller(Long claimId, Long sellerId, LocalDateTime processedAt)` | Seller wrapper·authorizeSellerAccess 호출 |
| `rejectBySeller` | `void rejectBySeller(Long claimId, Long sellerId, LocalDateTime processedAt)` | Seller wrapper·authorizeSellerAccess 호출 |
| `getClaim` | `ClaimResponse getClaim(String claimPublicId, Long buyerId)` | Buyer scope·변경 없음 |
| `listClaims` | `PagedResponse<ClaimSummaryResponse> listClaims(Long buyerId, int page, int size)` | Buyer scope·변경 없음 |
| `markCompleted` | `void markCompleted(Long claimId)` | actor 비의존·변경 없음 |
| `authorizeSellerAccess` | `private void authorizeSellerAccess(Claim claim, Long sellerId)` | **Seller 전용 권한 검증·Admin은 재사용 불가(seller_id 비교)** |
| `findClaim` | `private Claim findClaim(Long claimId)` | 내부 공통·Admin에서도 재사용 가능 |
| `clampSize` | `private int clampSize(int size)` | 내부 공통 |

> 실측: D-92 횡단 원칙 정합 — `approve`/`reject` primitive는 actor 비의존 시그니처 보존 완료. Admin wrapper(`approveByAdmin`·`rejectByAdmin`)는 `authorizeAdminAccess` 없이 직접 primitive 호출 또는 Admin role 검증 단락 추가. **~70% Service 재사용 D-87 Q3 실측 확인.**

### 12.5 SellerClaimControllerTest.java (전체 read·202줄)

> 실측: `@WebMvcTest(SellerClaimController.class)`. 8개 @Test — T1(승인 200)·T2(X-Seller-Id 누락 401)·T3(형식 오류 400)·T4(cross-tenant 404)·T5(미존재 404)·T6(상태 위반 422)·T7(거부 200)·T8(권한+상태 동시 위반 → 권한 우선 404). `@MockitoBean`: ClaimService·ClaimRepository·OrderItemRepository·SellerActorResolver. **→ AdminClaimControllerTest 8건 동일 패턴 신설 후보(Q1~Q3 확정 후).**

### 12.6 SellerClaimIntegrationTest.java (전체 read·232줄)

> 실측: `@SpringBootTest @AutoConfigureMockMvc @Transactional @RecordApplicationEvents`. Testcontainers MariaDB 11.4. 4건 @Test (I1 승인 200+APPROVED+ClaimApproved 1회·I2 거부 200+REJECTED+ClaimRejected 1회·I3 cross-tenant 404·I4 미존재 404). LT-02 try-finally 적용(seed 메서드·`FOREIGN_KEY_CHECKS 0/1`). `seedOrderItem(sellerId 파라미터화)`. **→ AdminClaimIntegrationTest β 패턴 4건 신설 후보(I1~I4 직접 재현·sellerId → adminId 없음·FK 부모 그래프 불필요 판단 후보).**

### 12.7 BuyerClaimController.java (전체 read·86줄)

> 실측: `@RestController @RequestMapping("/api/v1/claims")`. 3 endpoint: `@PostMapping`(request)·`@GetMapping("/{claimPublicId}")`·`@GetMapping`. **SellerClaimController와 동일 base path 공존 확정(Spring 라우팅 정상·HTTP method + sub-path 분기).** → AdminClaimController base path 공존 가능성은 Q6 해소 방법에 종속.

### 12.8 auth RBAC 인프라 실측

> 실측 (common/auth 디렉토리): **HeaderSellerActorResolver.java + SellerActorResolver.java 2파일만** — AdminActorResolver·HeaderAdminActorResolver 미존재 확정. SellerActorResolver Javadoc: "후속 Admin/Buyer 액터 resolver는 본 인터페이스를 공유하지 않고 별도 인터페이스로 분리한다" → Admin 전용 resolver 신설 필요(Q1·Q2 의제).

> 실측 (RoleCode enum·auth/enums/RoleCode.java·15줄): `SUPER_ADMIN·ADMIN_OPERATOR·BUYER·SELLER_OWNER·SELLER_MANAGER·SELLER_STAFF` 6값 확정(D-81 GRD-2 AUTH-3 DDL ENUM 정합). **enforcement(Spring Security·@PreAuthorize·SellerUser 조회 Service) 미연결 — 순수 데이터 모델만(실측 §3.14 carry-over).** Admin 역할: X-Admin-Id stub은 SUPER_ADMIN/ADMIN_OPERATOR 구분 없이 단일 헤더로 처리 가능(Q3 연계).

### 12.9 ClaimRepository.java (전체 read·32줄)

> 실측: `findByPublicId`·`existsActiveByOrderItemId(@Query)`·`findAllByRequestedBy(requestedBy, Pageable)`. **Admin scope 쿼리 0건 — findAll Page 버전·findAllForAdmin·findAllByStatus 부재.** Admin 목록 endpoint(Q8 β/γ) 채택 시 `Page<Claim> findAll(Pageable)` 또는 커스텀 쿼리 신규 후보. approve/reject 2개 한정(Q8 α) 채택 시 Repository 신규 0건.

### 12.10 Spring 라우팅 매트릭스 실측

| 컨트롤러 | base path | endpoint |
|---|---|---|
| BuyerClaimController | `/api/v1/claims` | POST(request)·GET({id})·GET(목록) |
| SellerClaimController | `/api/v1/claims` | POST `/{id}/approve`·POST `/{id}/reject` |
| AdminClaimController 후보 | `/api/v1/claims` | POST `/{id}/approve`·POST `/{id}/reject` **(SellerClaimController와 동일→충돌 후보)** |

> 실측: BuyerClaimController·SellerClaimController base path 공존 정상(HTTP method·sub-path 상이). **AdminClaimController가 `POST /{id}/approve`·`/{id}/reject`를 그대로 추가하면 SellerClaimController와 동일 @PostMapping → Spring AmbiguousHandlerMappingException 발생.** 해소 방법(headers 매핑·단일 통합 컨트롤러·다른 sub-path)은 Q6 핵심 의제.

### 12.11 Claim.java·ClaimStatus.java 메서드 목록 (전체 read)

| 항목 | 메서드 | actor 의존 |
|---|---|---|
| Claim | `create`·`markCompleted(processedAt)`·`approve(processedAt)`·`reject(processedAt)`·`transitionTo(private)` | **모두 actor 비의존** |
| ClaimStatus | `canTransitionTo(next)`: REQUESTED→APPROVED\|REJECTED·APPROVED→COMPLETED·REJECTED·COMPLETED→false | actor 비의존 |

> 실측: approve/reject는 `processedAt`만 받고 **actor 식별자 미포함**. D-92 횡단 원칙 정합 확인. Admin 진입 시 추가 전이 필요 없음 — REQUESTED→APPROVED·REJECTED 전이는 Admin도 동일 매트릭스 사용. Claim 엔티티·ClaimStatus 무변경 예상.

### 12.12 OrderItem.java·OrderItemRepository.java 메서드 목록 (전체 read)

> 실측 (OrderItem): `@Column(name="seller_id") private Long sellerId`·class-level `@Getter` → **`getSellerId()` 접근 가능**(`order` 필드만 `@Getter(AccessLevel.NONE)` 억제). Admin 권한 검증: **seller_id 비교 미적용** — Claim.orderItemId → OrderItem.getSellerId() 비교 없이 바로 approve/reject primitive 호출. `OrderItemRepository`: `findByOrderId`·`findByOrderIdIn`·`findByPublicId`·`findOrderIdById` — Admin scope 신규 메서드 불요(Q3 α seller 비교 skip 시).

### 12.13 GlobalExceptionHandler.java (전체 read·205줄)

> 실측: ClaimNotFoundException→404(CLAIM_NOT_FOUND)·ClaimInvalidStateException→422(CLAIM_STATE_INVALID)·UnauthenticatedException→401·MalformedRequestException/IllegalArgumentException→400. **클래스 Javadoc line 35 "403(Spring Security 후속 트랙)·... 제외" — 403 Forbidden 매핑 부재 확정**. Admin 무권한 처리: X-Admin-Id 누락→401(기 매핑)·Admin이 아닌 헤더값 처리→인증 실패만 처리 가능(403 신설 없이). Admin cross-tenant 개념 없음(전체 Claim 접근 허용·D-87 Q3)·GlobalExceptionHandler 무변경 예상.

### 12.14 ClaimApproved.java Javadoc 실측 (전체 read·24줄)

> 실측: ClaimApproved record Javadoc — "Track 10부터는 Seller 진입점 `ClaimService.approveBySeller`(D-92 Q3-sub a‴) 경유로도 동일 primitive에서 발행된다"·"소비자는 아직 부재다. D-92 Q8 β에 따라 Refund 자동 트리거는 본 트랙 범위 외이며... Track 10 범위에서는 발행만 보장하고 소비 검증은 수행하지 않는다." → **PR-B 진입 시 "Track 10-B Admin 진입점(`approveByAdmin` 경유)에서도 동일 primitive 발행" 보강 후보(Q10 γ).** 소비자 부재 언급은 유지.

### 12.15 Refund 자동 트리거·ClaimApproved 소비자 미포함 재확인

> 실측 (RefundService.java 메서드 목록): `initiate`·`markCompleted`·`markFailed`·`handleCallback`·`findByClaimId` — **ClaimApproved 이벤트 소비 메서드 0건**.

> 실측 (claim/handler 디렉토리): `ClaimCompletedHandler`·`ClaimRefundCompletedHandler`·`ClaimRejectedHandler`·`ClaimRequestedHandler` 4건 — **ClaimApprovedHandler 부재 확정(D-92 Q8 β carry-over·Refund 자동 트리거 후속 트랙).**

> PR-B Refund 자동 트리거 미포함 확정: D-92 운영 절차 "REQUESTED → APPROVED(Seller/Admin) → REFUND_PENDING → Refund 생성(Admin/Job)"·Refund Service 트랙 진입 시점 자동 변환(Q9 β carry-over).

---

## 13. 결정 라운드 의제 후보 (Q1~Q10·옵션 α·β·γ + 추천안 + 기조 4 정합)

> 기조: 1 운영 용이성·2 객관 판단·3 과잉문서 회피·4 과잉개발 회피. 추천안은 실측 데이터 기반 잠정안. **확정은 Claude.ai 결정 라운드 소관** (외부 검토 권장·S급·Q1·Q2·Q6 1순위).

### Q1. Admin 권한 인프라

| 옵션 | 내용 |
|---|---|
| α | **X-Admin-Id stub + AdminActorResolver 신설** (SellerActorResolver 1:1 패턴·common.auth 패키지) |
| β | SellerActorResolver seam 재사용 (Admin/Seller 동일 인터페이스) |
| γ | generic ActorResolver<T> 추출 (공통 seam·타입 파라미터화) |

**추천: α (X-Admin-Id stub + AdminActorResolver 신설).** 근거: SellerActorResolver Javadoc line 9 "후속 Admin/Buyer 액터 resolver는 본 인터페이스를 공유하지 않고 별도 인터페이스로 분리한다" 명시(실측 §12.1) → α 자연 정합. β는 Javadoc 명시 분리 위배. γ는 기조 4(과잉추상·현재 3개 독립 인터페이스가 더 명확). D-39·D-92 Q1 패턴 3중 1:1 정합.
**기조 정합**: 기조 2(실측 Javadoc 근거)·기조 4(신설 최소·generic 추상 회피).

### Q2. AdminActorResolver seam 분리 여부

| 옵션 | 내용 |
|---|---|
| α | **Admin 전용 별도 인터페이스** (AdminActorResolver·HeaderAdminActorResolver) |
| β | SellerActorResolver 재사용 |
| γ | generic ActorResolver 추출 |

**추천: α (Q1 α와 동일·SellerActorResolver Javadoc 명시 근거).** 별도 인터페이스를 신설해야 후속 Spring Security 교체 시 Admin·Seller resolver를 독립적으로 교체 가능.
**기조 정합**: 기조 1(후속 교체 운영 용이성)·기조 2(Javadoc 명시 근거).

### Q3. Admin 권한 모델

| 옵션 | 내용 |
|---|---|
| α | **Admin 전체 Claim 접근 허용** (SUPER_ADMIN·ADMIN_OPERATOR 구분 없이·seller_id 비교 skip) |
| β | SUPER_ADMIN/ADMIN_OPERATOR 분리 (SUPER_ADMIN=모든 권한·ADMIN_OPERATOR=승인/거절만) |
| γ | DENY 매트릭스 (일부 Claim type·status만 접근) |

**추천: α (전체 접근·구분 없음).** 근거: enforcement 미연결·RoleCode 데이터 모델만 존재(실측 §12.8). β는 RBAC enforcement 없이 구현 불가·Spring Security 종속. γ는 기조 4(과잉 정책·stub 단계 부적합). Admin은 seller_id 비교 없이 Claim 미존재만 404 — `approveByAdmin`·`rejectByAdmin` wrapper에서 `findClaim` 직접 호출 후 approve/reject primitive 위임.
**기조 정합**: 기조 4(enforcement 미연결 상태 세분화 금지)·기조 2(실측 인프라 현황).

### Q4. PR 분할

| 옵션 | 내용 |
|---|---|
| α | **Admin 단독 1 PR** (Track 10-B) |
| β | Seller + Admin 통합 1 PR (Track 10 재오픈) |
| γ | Admin + 목록 + 기타 확장 포함 대형 PR |

**추천: α (Admin 단독 1 PR).** 근거: D-92 Q2 α (Seller 단독 1 PR) 선례 직접 재적용. β는 Track 10 이미 머지·PR 재오픈 불가. γ는 기조 4(목록 ClaimRepository 신규 비중 증가·D-88 명시 범위 초과).
**기조 정합**: 기조 1(리뷰 단위 적정·D-92 Q2 패턴)·기조 4(Admin 최소 응집).

### Q5. Admin 권한 검증 위치·HTTP

| 옵션 | 내용 |
|---|---|
| α | **Service 진입부·ClaimNotFoundException 404 재사용** (미존재만 404·seller 비교 없음) |
| β | Service 진입부·Admin role 검증 추가 (SUPER_ADMIN/ADMIN_OPERATOR 체크) |
| γ | Controller 진입부·HTTP 분기 |

**추천: α (Service 진입부·404 재사용).** 근거: D-89 Q8·D-92 Q3 Service 진입부 책임 원칙 정합. Admin은 전체 접근 허용(Q3 α) → seller_id 비교 단락 없음·Claim 미존재 → 404(정보 노출 회피·GlobalExceptionHandler 무변경). 403 미매핑 확정(실측 §12.13)·Spring Security 후속 트랙. β는 RBAC enforcement 미연결 상태 stub 구현 불가.
**기조 정합**: 기조 2(D-92 Q3·GlobalExceptionHandler 실측 근거)·기조 4(403 신설 회피).

### Q6. endpoint URL 패턴 및 Spring 라우팅 충돌 해소 ★최우선 의제

| 옵션 | 내용 |
|---|---|
| α | **headers 매핑 분기** — SellerClaimController에 `headers = "X-Seller-Id"` 추가·AdminClaimController에 `headers = "X-Admin-Id"` 추가 → Spring header 조건부 매핑으로 충돌 해소 |
| β | **단일 통합 컨트롤러** — SellerClaimController를 AdminSellerClaimController로 교체·헤더 감지 내부 분기 |
| γ | Admin에 별도 sub-path — `/api/v1/claims/{id}/admin/approve`·`/reject` (D-40 prefix 금지와의 긴장) |

**추천: α (headers 매핑 분기).** 근거: Spring MVC `@RequestMapping(headers = "X-Seller-Id")` 조건부 라우팅 정상 지원·Seller/Admin 컨트롤러 독립 유지(D-40 β′ Controller 액터 분리 정합). 단 기존 SellerClaimController @PostMapping에 `headers` 조건 추가가 필요(수정 1건·기존 테스트 헤더 주입 이미 존재). β는 D-40 Controller 분리 약화·단일 컨트롤러에 액터 분기 혼입. γ는 D-40 액터 중립 원칙과 긴장(admin 부분 prefix).

**⚠ 중요**: SellerClaimController `@PostMapping` 수정 시 SellerClaimControllerTest의 MockMvc 호출 헤더 조건 재검증 필요 — 기존 테스트가 `.header("X-Seller-Id", "1")` 포함하므로 headers 조건 추가 시 테스트 회귀 없음 예상. **외부 검토 1순위.**
**기조 정합**: 기조 1(Spring 표준 라우팅·D-40 유지)·기조 2(실측 충돌 근거).

### Q7. DTO 재사용

| 옵션 | 내용 |
|---|---|
| α | **ClaimResponse 재사용** (D-92 Q5 α 패턴 1:1) |
| β | Admin 전용 AdminClaimResponse 신설 (requestedBy 포함) |
| γ | 혼합 (approve/reject α·목록 β) |

**추천: α (ClaimResponse 재사용).** 근거: 승인/거절 응답은 상태 전이 결과만 필요(publicId·status·processedAt). requestedBy 노출은 D-89 Q7 보안·requestedBy 미노출 정합. Admin 목록(Q8 β/γ) 미채택 시 α 신규 DTO 0건. D-92 Q5 α 패턴 재적용.
**기조 정합**: 기조 4(DTO 신설 0건·Q8 α 종속).
...
|---|---|
| α | **2개 (approve·reject)** |
| β | 3개 (approve·reject·전체 목록) |
| γ | 4개 (approve·reject·단건·전체 목록) |

**추천: α (2개·approve·reject).** 근거: D-88 Q2 "Seller/Admin 승인·거절 endpoint" 명시 범위 한정. Admin 목록은 ClaimRepository Admin scope 쿼리 부재(실측 §12.9)·신규 쿼리 동반 = 범위 확장(기조 4). β/γ는 Admin 워크플로우(목록에서 건별 승인) UX 합리적이나 ClaimRepository 신규 비중 증가. **목록 필요성은 결정 라운드 업무 정책 항목.**
**기조 정합**: 기조 4(D-88 명시 범위 한정)·기조 1과 트레이드오프 → 외부 검토 의제.

### Q9. Refund 자동 트리거 본 PR 포함 여부

| 옵션 | 내용 |
|---|---|
| α | 본 PR 포함 (ClaimApproved→Refund 생성 핸들러 신설) |
| **β** | **본 PR 미포함** (D-92 Q8 β carry-over) |

**추천: β (미포함·D-92 Q8 β carry-over).** 근거: D-92 운영 절차 박제 "REQUESTED → APPROVED(Seller/Admin) → REFUND_PENDING → Refund 생성(Admin/Job)". ClaimApproved 소비자 부재 확정(실측 §12.15). Refund Service 트랙 진입 시점 ClaimApproved→RefundCreated 자동 변환 구성.
**기조 정합**: 기조 4(Refund Service 미성숙·D-92 명시 이연).

### Q10. Javadoc·D-50 매트릭스 동반 여부

| 옵션 | 내용 |
|---|---|
| α | 본 PR 동반 (ClaimApproved Javadoc + D-50 둘 다) |
| β | 별도 트랙 (둘 다) |
| γ | **부분 동반** (ClaimApproved Javadoc Admin endpoint 언급 보강·D-50 별도 트랙) |

**추천: γ (부분 동반).** 근거: ClaimApproved Javadoc 현재 "Track 10부터는 Seller 진입점 경유" → PR-B에서 "Track 10-B Admin 진입점(`approveByAdmin`) 경유에서도 동일 primitive 발행"으로 1줄 보강(실측 §12.14). D-92 Q7 γ 패턴 재적용. D-50 별도 트랙 이연 유지(D-92 후속).
**기조 정합**: 기조 2(ClaimApproved Javadoc 실측 근거)·기조 4(D-50 scope 혼입 회피).

### 의제 요약·외부 검토 권장

| Q | 추천(잠정) | 외부 검토 권장도 |
|---|---|---|
| Q1 Admin 권한 인프라 | α X-Admin-Id stub + AdminActorResolver | **상** (새 seam 패턴·D-92 횡단 원칙 첫 재사용) |
| Q2 AdminActorResolver 분리 | α 별도 인터페이스 | **상** (Q1 종속·Javadoc 명시 근거) |
| Q3 Admin 권한 모델 | α 전체 접근·구분 없음 | 중 |
| Q4 PR 분할 | α Admin 단독 1 PR | 하 (D-92 Q2 α 선례) |
| Q5 권한 검증 위치·HTTP | α Service 진입부·404 재사용 | 중 (D-92 Q3 패턴 재적용) |
| Q6 URL·라우팅 충돌 해소 | α headers 매핑 분기 | **최상** (라우팅 충돌·SellerClaimController 수정 동반) |
| Q7 DTO 재사용 | α ClaimResponse 재사용 | 하 (D-92 Q5 α 패턴) |
| Q8 endpoint 개수 | α 2개 | 중 (Admin UX 트레이드오프) |
| Q9 Refund 자동 트리거 | β 미포함 | 하 (D-92 Q8 β 확정) |
| Q10 Javadoc/D-50 | γ 부분 | 하 (D-92 Q7 γ 패턴 재적용) |

---

## 14. WARN

### WARN-1. Spring 라우팅 충돌 후보 [PR-B 진입 전 해소 의무·Q6 결정 연계]

**내용**: SellerClaimController `@PostMapping("/{claimPublicId}/approve")`·`reject`가 동일 base path에 이미 선점됨. AdminClaimController에 동일 경로 추가 시 Spring `AmbiguousHandlerMappingException` 발생·PR-B 빌드 실패 확실. **실측 근거(§12.10): BuyerClaimController·SellerClaimController 공존은 HTTP method + sub-path 분기로 정상이나 AdminClaimController는 동일 method + 동일 sub-path 충돌.**
**해소**: Q6 결정(α headers 매핑·β 단일 컨트롤러·γ 다른 sub-path) 확정 후 진입.

### WARN-2. AdminActorResolver 미존재 [신설 필요·Q1·Q2 결정 연계]

**내용**: common/auth 디렉토리에 HeaderSellerActorResolver + SellerActorResolver만 존재(실측 §12.8). AdminActorResolver·HeaderAdminActorResolver 부재 확정. SellerActorResolver Javadoc "Admin resolver는 별도 인터페이스로 분리한다" 명시.
**해소**: Q1 α 채택 시 AdminActorResolver interface + HeaderAdminActorResolver @Component 신설(SellerActorResolver 패턴 1:1).

### WARN-3. 403 Forbidden Admin 권한 실패 매핑 부재 [carry-over·Q5 연계]

**내용**: GlobalExceptionHandler 403 Forbidden 매핑 부재 확정(실측 §12.13·line 35 "Spring Security 후속 트랙 제외"). Admin 무권한 처리: X-Admin-Id 누락→401(기 매핑)·Admin 아닌 값 처리→인증 실패(401)만 처리 가능. RBAC enforcement 미연결 상태에서 역할 검증 불가.
**해소**: Q5 α (admin 전체 접근·404만 재사용) 채택 시 GlobalExceptionHandler 무변경·403 신설 불요. Q3 β(SUPER_ADMIN/ADMIN_OPERATOR 분리) 채택 시 403 신설·Spring Security 종속 → 기조 4 위배 위험.

### WARN-4. ClaimApproved 소비자 부재 [carry-over D-92·Q9 연계]

**내용**: claim/handler 실측(§12.15): ClaimApprovedHandler 0건. RefundService ClaimApproved 소비 메서드 0건. D-92 Q8 β carry-over 확정.
**해소**: Q9 β 유지(본 PR 미포함)·Refund Service 트랙 진입 시점.

### WARN-5. ClaimRepository Admin scope 쿼리 부재 [Q8 β/γ 채택 시 신설 필요]

**내용**: ClaimRepository 실측(§12.9): `findAllByRequestedBy`(Buyer scope)만. Admin용 Page<Claim> findAll 또는 status/type 필터 쿼리 부재. Q8 α(approve/reject 2개) 채택 시 영향 없음. Q8 β/γ(목록 포함) 채택 시 신규 Repository 메서드 후보.
**해소**: Q8 결정 종속. Q8 α 유지 시 carry-over 불필요.

---

## 15. 영향 범위 예비 목록 (정찰 룰 #3·최종 확정은 결정 라운드 종료 후)

> 본 절은 **예비 목록**이며 최종 확정 표는 WARN 전건 해소·Q1~Q10 결정 확정 후 결정 라운드에서 박제한다.

### 15.1 신규 파일 후보

| 후보 | 책임 | 종속 의제 |
|---|---|---|
| `common/auth/AdminActorResolver.java` | Admin 액터 식별자 해석 인터페이스 | Q1·Q2 |
| `common/auth/HeaderAdminActorResolver.java` | X-Admin-Id 헤더 기반 구현체 | Q1·Q2 |
| `claim/controller/AdminClaimController.java` | approve·reject endpoint·X-Admin-Id resolve | Q3·Q4·Q6 |
| `test/.../claim/controller/AdminClaimControllerTest.java` | @WebMvcTest·HTTP 매트릭스 8건 | (필수) |
| `test/.../claim/integration/AdminClaimIntegrationTest.java` | E2E·X-Admin-Id·권한 격리·LT-02 | (필수) |

### 15.2 수정 파일 후보

| 후보 | 변경 | 종속 의제 |
|---|---|---|
| `claim/service/ClaimService.java` | approveByAdmin·rejectByAdmin wrapper 신설(authorizeAdminAccess 또는 단락 없음) | Q3·Q5 |
| `claim/controller/SellerClaimController.java` | `@PostMapping(headers = "X-Seller-Id")` 헤더 조건 추가(Q6 α 채택 시) | Q6 |
| `claim/event/ClaimApproved.java` | Javadoc Admin endpoint 경유 1줄 보강 | Q10 γ |

### 15.3 문서 갱신 후보

| 후보 | 변경 |
|---|---|
| decisions.md | D-93(가칭) Track 10-B Admin endpoint 진입 결정 박제 |
| docs/track-10/recon-report.md | §15+ 갱신(결정 흡수·WARN 해소·영향 범위 최종 확정) |

### 15.4 영향 0건 예상 (현재 가정)

Claim.java·ClaimStatus.java·OrderItem.java·Entity·DDL·Flyway·GlobalExceptionHandler(Q5 α·Q6 α 채택 시)·ClaimRepository(Q8 α 채택 시). LT-01·03 처치 의무 없음(신규 Entity 0건 예상). LT-02 의무 있음(신규 통합 테스트 FK_CHECKS try-finally·SellerClaimIntegrationTest 패턴 1:1).

---

## 16. 후속 자연 진입 트랙 식별자 (정찰 룰 #4)

| 트랙 | 범위 | 출처 |
|---|---|---|
| **Refund Service 트랙** | ClaimApproved → RefundCreated 자동 변환·CLM-3 | D-92 Q8 β·후속 트랙·운영 절차 |
| **NotificationLog 트랙** | Seller/Admin 승인/거절 알림 source·ClaimApproved/Rejected 소비 | D-92 후속 |
| **Track 11** | Claim RETURN/EXCHANGE 확장·Delivery 수거·재출고·ClaimReasonCode +4값 | D-88 후속·D-90 후속 |
| **Track 12+** | Observability·이벤트 핸들러 멱등성·correlationId/eventId 일괄 | D-90 후속 |
| **Spring Security 트랙** | SecurityFilterChain·UserDetailsService·auth RBAC enforcement·X-*-Id stub 일괄 대체 | D-39 범위 외·D-88 "Spring Security 진입 후보" |
| **별도 트랙** | D-50 Validation 매트릭스 정정·CheckoutIntegrationTest/RefundWebhookIntegrationTest LT-02 보정 | D-90 후속·D-89 Q9 |

식별자 **6건** (D-92 §7과 동일).

---

## 17. 진입 조건 체크리스트 (Track 10-B Admin endpoint)

- [x] 정찰 룰 #1 (파일 전체 read 의무) — 15파일군 전체 read 완료 (§12.1~§12.15)
- [x] 정찰 룰 #2 (메서드 목록 전체 확인) — ClaimService 11개·Claim 5개·ClaimStatus 1개·Repository 메서드 목록 §12 박제
- [x] 정찰 룰 #3 (§15 영향 범위 예비 목록만·최종 확정 미박제) — §15 예비 표기
- [x] 정찰 룰 #4 (후속 자연 진입 트랙 식별자 명시) — §16 6건 박제
- [x] 실측 우선·추측 금지 (MCP read 실측 의무) — §12 각 항목 1줄 실측 인용
- [x] 외부 편집 검증 의무 (read-only·코드/테스트/DDL 변경 0건)
- [x] SoT 정독 완료 (decisions.md D-39·D-40·D-87~D-92·live-traps.md LT-01~03·recon-report.md §1~§9·§4.8·§5.6·§6.5)
- [x] 결정 라운드 의제 도출 (Q1~Q10·옵션 α/β/γ·추천안·기조 정합)
- [ ] 결정 라운드 미진입 상태 (외부 검토 권장·Q1·Q2·Q6 최우선)

**진입 가능 상태**: Track 10-B 정찰 산출물 완성. 결정 라운드 (Q1~Q10·WARN-1 해소 의무·외부 검토 S급·Q6 최우선) 후 PR 진입.
...
| 항목 | 핵심 |
|---|---|
| D-87 Q3 | Admin API = 별도 트랙·~70% Service 재사용 |
| D-88 Q2·후속 | Seller/Admin endpoint Track 10 이연·Spring Security 진입 후보 |
| D-89 Q3·Q7·Q8 | ClaimInvalidStateException 422·requestedBy 미노출·소유권 Service 2단계·404 정보 노출 회피 |
| D-90 Q2·백로그 | Refund 자동 트리거 Track 10 이연·ClaimApproved 발행 유지 |
| D-91 | 통합 테스트 FK 부모 그래프 시드 의무 |
| D-92 전건 | Track 10 Seller 완료·횡단 원칙·wrapper 캡슐화·WARN-5 테스트 경계·Q8 β Refund 미포함·Track 10-B 재사용 의무 |
| D-39 | X-Buyer-Id stub·Seller/Admin 인증 후속 트랙 |
| D-40 | URL 액터 중립·`/api/seller/` prefix 금지·Controller 액터별 분리 |
| invariants §2.13 | CLM-1~5 (CLM-3 Refund는 승인 후 생성) |
| state-machine §2 | Claim REQUESTED→APPROVED/REJECTED("관리자/판매자 승인·거절") |
| live-traps LT-02 | FK_CHECKS try-finally·신규 통합 테스트 의무 |
| SellerActorResolver Javadoc | "Admin resolver는 별도 인터페이스" 명시·Q1·Q2 근거 |

---

## 19. 결정 라운드 확정 결과 (Track 10-B·2026-06-30·D-93 박제 동반)

§13 정찰 의제 Q1~Q10 + 외부 검토 신규 Q11에 대한 결정 라운드(외부 검토 1·2차)가 종료되었다. **본문 전건은 decisions.md D-93(line 3500~3621·`[ACTIVE]`)에 박제되며, 본 절은 정찰 추천 → D-93 확정 델타·근거만 요약한다.** D-93은 정찰 단계 동반 박제 산출물로 구현 PR(`feat/track-10-pr-b`)에 자연 이월된다.

### 19.1 Q1~Q11 확정안 (정찰 추천 vs D-93 확정)

| Q | 정찰 추천(§13) | D-93 확정 | 델타 |
|---|---|---|---|
| Q1 권한 인프라 | α X-Admin-Id stub + AdminActorResolver | α | — |
| Q2 AdminActorResolver 분리 | α 별도 인터페이스 | α | — |
| Q3 Admin 권한 모델 | α 전체 접근 | α 전체 접근·stub 단계 한정 명시 | 범위 명시 추가 |
| Q4 PR 분할 | α Admin 단독 1 PR | α | — |
| Q5 권한 검증 위치·HTTP | α Service 진입부·404 | α Service 진입부·404 단일 처리 | — |
| Q6 URL·라우팅 충돌 | α headers 매핑 분기 | **γ′ `/api/v1/admin/claims`** | **변경(외부 검토 2차)** |
| Q7 DTO 재사용 | α ClaimResponse 재사용 | α | — |
| Q8 endpoint 개수 | α 2개 | α 2개 | — |
| Q9 Refund 자동 트리거 | β 미포함 | β carry-over | — |
| Q10 Javadoc·D-50 | γ 부분 동반 | γ 부분 동반 | — |
| Q11 SecurityContext seam 유지 | (정찰 미도출) | 이연·백로그 | 신규 의제(외부 검토) |

10건 중 9건 정찰 추천 유지·**Q6만 α(headers 매핑) → γ′(/admin prefix) 변경**·Q11 신규 1건.

### 19.2 외부 검토 1·2차 흡수 흐름

- **1차**: Q1·Q2 α 동의·Q6 β′(단일 컨트롤러 + delegate) 권장·Q3 stub 단계 한정 권장·Q5 표현 수정·Q11 신규 의제 제시
- **2차(기조 4 재평가)**: β′ 자체 철회(D-92 wrapper와 중복 추상화)·γ′ 조건부 1순위(D-40 원문 재확인 조건)·α(headers 매핑) 실측 후 판단으로 하향·Q11 이연·정찰 §15 영향 범위 E1 5건 철회·E2 Q10 γ 충돌 해소
- **D-40 본문 실측**: γ′ 채택 전제(D-40 원문 `/admin` 명시 부재) 확인 완료(§19.3)

### 19.3 D-40 본문 실측 인용 (Q6 γ′ 근거)

decisions.md D-40(line 1103·Track 4·[ACTIVE]) 결정 1 실측:
...
**실측 확정**: D-40 prefix 금지 명시는 `/buyer`·`/seller` 2건 한정·`/admin` 명시 부재. 또한 결정 2는 `AdminOrderController`(후속) 분리를 사전 박제(`BuyerOrderController`·`SellerOrderController`·`AdminOrderController`). → γ′ `/api/v1/admin/claims` 채택은 D-40 본문 직접 위배 아님(D-93 Q6 정합). WARN-1(라우팅 충돌)은 base path 분리로 해소·SellerClaimController 무변경.

### 19.4 WARN-1~5 해소 상태

| WARN(§14) | 해소 |
|---|---|
| WARN-1 라우팅 충돌 | 해소·Q6 γ′ `/api/v1/admin/...` prefix 분리·base path 충돌 회피·SellerClaimController 무변경 |
| WARN-2 AdminActorResolver 미존재 | 해소·Q1·Q2 α 신설(AdminActorResolver·HeaderAdminActorResolver) |
| WARN-3 403 매핑 부재 | 해소·Q5 α 404 단일 처리·GlobalExceptionHandler 무변경 |
| WARN-4 ClaimApproved 소비자 부재 | 범위 외 확정(이연)·Q9 β carry-over·Refund Service 트랙 |
| WARN-5 ClaimRepository Admin scope 부재 | 해소·Q8 α(2 endpoint)·신규 쿼리 0건·불요 확정 |

### 19.5 본문 위임

본 절은 요약이며, 결정 근거·대안 검토(기각 8건)·영향 범위(신규 5·수정 2·문서 1·무변경 확정)·관련 결정·후속 트랙 전건은 decisions.md D-93에 위임한다. 구현 산출물·회귀 결과는 D-93 §영향 범위·PROGRESS.md STEP Track 10 PR-B 구현 블록 참조.
