...
/**
 * {@link ClaimRejectedHandler} 단위 검증(D-90 Q3 α-1·claim-lock release). CANCEL 한정 OrderItem CANCEL_REQUESTED → PAID
 * 환원·Order.status 재계산 위임과 type·null·비대상 가드를 mock 경계에서 검증한다.
 */
...
class ClaimRejectedHandlerTest {
    ...
    @Test
    @DisplayName("onClaimRejected: CANCEL_REQUESTED OrderItem → PAID claim-lock release·recalculateStatus 위임")
    void onClaimRejected_cancelRequested_releasesToPaid() {
    ...
    @Test
    @DisplayName("onClaimRejected: type=RETURN → 미처리(findById·재계산 없음)")
    void onClaimRejected_nonCancel_noOp() {
    ...
    @Test
    @DisplayName("onClaimRejected: 비대상 상태(PAID) → no-op(전이·재계산 없음)")
    void onClaimRejected_notCancelRequested_noOp() {
        OrderItem orderItem = mock(OrderItem.class);
