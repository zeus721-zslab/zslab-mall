...
/**
 * PaymentFailed(E3) → Inventory 예약 해제 핸들러(Track 17 PR-B·D-101 §3). {@link PaymentFailed}를 소비해 orderId로
 * OrderItem을 재조회한 뒤 각 품목의 예약을 {@link InventoryService#release}로 해제한다.
 *
 * <p><b>실행 시점(D-101 §3·D-75)</b>: {@code @TransactionalEventListener(AFTER_COMMIT)} + {@code REQUIRES_NEW}로 결제 실패
 * 커밋 후 별도 트랜잭션에서 해제한다.
 *
 * <p><b>이중 방어(D-100 Q1 γ·D-101 §6·§4 갱신)</b>: 1차 핸들러 가드 = {@link InventoryRepository#findByVariantId}
 * read-only 조회로 잔여 reserved == 0이면 skip(멱등·§4 갱신 read-only 2 예외에 해당·쓰기는 아님), 2차 도메인 가드 =
 * release INV-3(예약 초과 해제) 위반 시 InventoryInvariantViolationException throw.
 *
...
public class InventoryPaymentFailedHandler {
        ...
        try {
            ...
            for (OrderItem item : items) {
                ...
                if (inventory == null || inventory.getQuantityReserved() == 0) {
                    log.info("[Inventory] event=PaymentFailed target_id={} action=skip reason=reserved_zero variant_id={}",
                            item.getId(), item.getVariantId());
                    continue;
                }
                inventoryService.release(item.getVariantId(), item.getQuantity());
                log.info("[Inventory] event=PaymentFailed target_id={} action=release variant_id={} qty={}",
            ...
            log.warn("[Inventory] event={} target_type={} target_id={} action=manual_review correlationId={} handler={}",
                    "PaymentFailed", "ORDER", event.orderId(), MDC.get("correlationId"), this.getClass().getSimpleName(), exception);
