 ...
 *
 * <p><b>대상(Track 96-6)</b>: 탈퇴 회원({@code user.withdrawn_at IS NOT NULL})은 제외한다 — 관리자 전체 재산정 API와 일 배치 스케줄러가
 * 같은 경로를 쓰므로 동일하게 적용된다. 등급 잠금({@code grade_locked_until}) 존중은 {@link GradeService} 가드 그대로다.
 */
@Slf4j
@Service
...
public class GradeRecalculationBatchService {
    ...
    /**
     * 활성(미탈퇴) buyer 전체의 등급을 buyer별 독립 트랜잭션으로 재산정한다. lock 기간 buyer는 recalculate 내부에서 skip되며 success로
     * 집계된다.
     *
     * @return 총 대상·성공·실패 카운트 요약
     */
    public GradeRecalculationResult recalculateAll() {
        List<Long> buyerIds = buyerProfileRepository.findActiveBuyerIds();
