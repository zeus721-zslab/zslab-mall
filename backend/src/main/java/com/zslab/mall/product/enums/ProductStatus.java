 ...
 * {@link #canTransitionTo}로 하며(Product mutator가 이를 가드로 사용·SettlementStatus 동일 패턴).
 *
 * <p><b>거부 철회(Track 101-A)</b>: REJECTED는 더 이상 종료 상태가 아니다. 운영자가 오거부를 되돌릴 수 있도록 REJECTED → PENDING
 * 1건을 추가했다(관리자 전용·사유 필수). 철회 후에는 PENDING의 기존 동작(승인·거부·셀러 수정 후 재요청)을 그대로 쓴다 —
 * 재심사 전용 상태를 새로 만들지 않는다.
 *
...
public enum ProductStatus {
     ...
     *
     * @param next 목표 상태
     * @return 합법 전이면 true(PENDING→SALE·PENDING→REJECTED·REJECTED→PENDING·SALE→STOPPED·STOPPED→SALE만·그 외 전부 false)
     */
    public boolean canTransitionTo(ProductStatus next) {
        return switch (this) {
            case PENDING -> next == SALE || next == REJECTED;
            // 거부 철회(Track 101-A). 되돌릴 곳은 거부 직전 상태인 PENDING 하나뿐이다.
            case REJECTED -> next == PENDING;
            case SALE -> next == STOPPED;
            case STOPPED -> next == SALE;
            // 그 외 상태(DRAFT·APPROVED·HIDDEN)에서의 전이는 범위 밖 — 전부 불가
            default -> false;
        };
    }
}
