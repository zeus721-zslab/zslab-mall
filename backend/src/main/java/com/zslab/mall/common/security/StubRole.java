...
/**
 * Stub 인증 파이프라인(Track 31 Phase 1)이 다루는 액터 역할 3종. Authorization 헤더 {@code "Stub <role>:<id>"}의
 * role 토큰과 Spring Security 권한(ROLE_ 프리픽스)을 잇는다.
 *
...
public enum StubRole {
     ...
     */
    public static StubRole fromToken(String token) {
        return StubRole.valueOf(token.trim().toUpperCase());
    }
}
