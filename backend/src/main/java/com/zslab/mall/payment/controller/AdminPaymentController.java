...
import com.zslab.mall.payment.controller.request.AdminPaymentMarkCancelledRequest;
 ...
 *
 * <p>HTTP 책임만 가진다(D-40 β′): 요청 검증·액터 해소·감사 컨텍스트 조립·Service 위임·응답 조립만 수행한다. Admin은 전체 접근이므로
 * 권한 검증 단락이 부재하며(D-93 Q3·Q5) paymentPublicId 미존재만 404({@link PaymentNotFoundException})다. publicId 해소·감사 적재는
 * {@link AdminPaymentCommandService}(Track 89-A) 책임이다.
 *
...
public class AdminPaymentController {
    ...
    public AdminPaymentController(
            AdminPaymentCommandService adminPaymentCommandService,
            AdminActorResolver adminActorResolver,
            ActorRoleResolver actorRoleResolver) {
        this.adminPaymentCommandService = adminPaymentCommandService;
        this.adminActorResolver = adminActorResolver;
        this.actorRoleResolver = actorRoleResolver;
    }

    /**
     * Admin 수동 결제 취소(D-113). 미존재 paymentPublicId → 404·사유 누락/200자 초과 400. 전액 환불 완료 시 PAID→CANCELLED, 부분·미환불은
     * 상태 유지 NO-OP(D-71 가드 상속), 이미 CANCELLED면 멱등 NO-OP다. NO-OP도 예외 없이 200 + 현재 결제 상태를 반환한다.
     * 사유는 상태가 바뀐 경우에만 감사 로그에 남는다(Track 89-A).
     */
    ...
    public AdminPaymentMarkCancelledResponse markCancelled(
            ...
            @RequestBody @Valid AdminPaymentMarkCancelledRequest request,
        ...
        // 서비스는 @Transactional 종료 후 취소 반영 Payment를 반환한다. 스칼라 status만 읽으므로 OSIV off에서도 재조회 불요.
        Payment cancelled = adminPaymentCommandService.markCancelled(paymentPublicId, request.reason(), auditContext);
