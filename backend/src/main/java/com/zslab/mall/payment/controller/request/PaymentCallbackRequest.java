 ...
 *
 * <p>{@code pgTid}·{@code metadata}는 실패·취소 콜백 등에서 비어 있을 수 있어 필수를 강제하지 않는다(형식만·도메인 검증은 Service).
 * pgTid 길이는 컬럼(payment.pg_tid VARCHAR(100))에 맞춰 선검증한다 — 초과분이 DB 오류(500)로 새던 것을 400으로 교정(Track 93 D-198).
 ...
 * @param pgTid              PG 거래 ID(미발급 케이스 null 허용·최대 100자)
