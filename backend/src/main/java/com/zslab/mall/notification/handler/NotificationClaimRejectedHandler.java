...
/**
 * ClaimRejected → SMS NotificationLog 적재 핸들러(Track 80 D-169·C5). {@link ClaimRejected}를 소비해
 * {@link NotificationService#recordClaimRejected}로 구매자 거부 SMS를 적재·발송한다.
 *
 * <p><b>클래스명(D-74·D-95)</b>: {@code claim/handler/ClaimRejectedHandler(품목 스냅샷 원복)}와 동명 충돌을 피하려 {@code Notification} prefix를 붙인다.
 *
...
public class NotificationClaimRejectedHandler {
    ...
    public void handle(ClaimRejected event) {
        try {
            notificationService.recordClaimRejected(event);
        } catch (RuntimeException exception) {
            log.warn("[Notification] event={} target_type={} target_id={} action=manual_review correlationId={} handler={}",
                    "ClaimRejected", "CLAIM", event.claimId(), MDC.get("correlationId"), this.getClass().getSimpleName(), exception);
