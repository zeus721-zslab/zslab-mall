...
- **선택지:** 조건부 UPDATE 차감 / 낙관적 락 / **비관적 락(채택)**
...
- 실제 구현체 없이 `mock` 외 값을 넣으면 **기동 시점에 실패**합니다. 조용히 모의 구현으로 되돌아가는 일이 없습니다.
...
- **화면도 채점합니다:** 구매자 화면을 6개 항목(디자인·일관성·직관성·편의성·접근성·성능)으로 채점하고, 목표(평균 95)에 못 미치는 항목만 보완했습니다. 기준선 76.4 → 최종 95.8. 성능은 Lighthouse 모바일 측정으로 재고, 게이트웨이 응답 압축 하나로 성능 71 → 87을 얻었습니다.
